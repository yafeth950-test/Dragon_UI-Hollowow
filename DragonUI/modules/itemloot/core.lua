...
-- /* assets */
local assets = [[Interface\AddOns\DragonUI\modules\itemloot\assets\]];
...
private.alertQueue = LootAlertFrameMixIn.alertQueue;
...
function LootAlertFrameMixIn:AdjustAnchors()
	...
	local anchor = _G["DragonUI_LootRollAnchor"] or (addon.LootRollModule and addon.LootRollModule.anchorFrame) or DungeonCompletionAlertFrame1;
	for i=1, LOOTALERT_NUM_BUTTONS do
		local button = self.alertButton[i];
		...
		if button and button:IsShown() then
			if button.waitAndAnimOut:GetProgress() <= 0.74 then
				if not previousButton or previousButton == button then
					if anchor then
						button:SetPoint("CENTER", anchor, "CENTER", point_x, point_y);
					else
...
function LootAlertFrame_OnLoad(self)
	...
	if not addon.IsModuleEnabled or addon:IsModuleEnabled("itemloot") then
...
function LootAlertFrame_OnEvent(self, event, ...)
	if addon.IsModuleEnabled and not addon:IsModuleEnabled("itemloot") then
		return;
	end
