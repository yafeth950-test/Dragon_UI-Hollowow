--[[
================================================================================
DragonUI_Options - 繁體中文本地化檔案
================================================================================
選項面板基礎本地化：標籤、描述、分區標題、下拉選單值、列印資訊、彈出文字。

新增字串時：
1. 在此處新增 L[<你的鍵>] = true
2. 在選項程式碼中使用 L["你的字串"]
3. 為其他本地化檔案新增翻譯
================================================================================
]]

local L = LibStub("AceLocale-3.0-DragonUI"):NewLocale("DragonUI_Options", "zhTW")
if not L then return end

L["Collector"] = "收集器"
L["Minimap Buttons Collector"] = "小地圖按鈕收集器"
L["Circle"] = "圓形"
L["Arrow"] = "箭頭"

-- ============================================================================
-- 通用 / 面板
-- ============================================================================

L["DragonUI"] = "DragonUI"
L["Use the tabs on the left to configure modules, action bars, unit frames, minimap, and more."] = "使用左側標籤頁配置模組、動作條、單位框架、小地圖等。"
L["Editor Mode"] = "編輯模式"
L["Exit Editor Mode"] = "退出編輯模式"
L["KeyBind Mode Active"] = "按鍵繫結模式已啟用"
L["Move UI Elements"] = "移動介面元素"
L["Cannot open options during combat."] = "戰鬥中無法開啟選項。"
L["Open DragonUI Settings"] = "開啟DragonUI設定"
L["Open the DragonUI configuration panel."] = "開啟DragonUI配置面板。"
L["Use /dragonui to open the full settings panel."] = "輸入 /dragonui 以開啟完整設定面板。"

-- 快速操作
L["Quick Actions"] = "快速設定"
L["Jump to popular settings sections."] = "快速跳轉到常用設定分割槽。"
L["Action Bar Layout"] = "動作條佈局"
L["Configure dark tinting for all UI chrome."] = "為所有介面裝飾元素配置深色著色。"
L["Full-width health bar that fills the entire player frame."] = "填滿整個玩家框架的寬體生命條。"
L["Add a decorative dragon to your player frame."] = "為你的玩家框架新增裝飾性的龍。"
L["Heal prediction, absorb shields and animated health loss."] = "治療預估、吸收護盾和動態生命值損失。"
L["Change columns, rows, and buttons shown per action bar."] = "更改每個動作條顯示的列、行和按鈕數量。"
L["Switch micro menu icons between colored and grayscale style."] = "在彩色和灰度風格之間切換微型選單圖示。"
L["About"] = "關於"
L["Bringing the retail WoW look to 3.3.5a, inspired by Dragonflight UI."] = "受巨龍時代UI啟發，為3.3.5a版本帶來正式服《魔獸世界》的外觀。"
L["Created and maintained by Neticsoul, with community contributions."] = "由Neticsoul建立和維護，並有社群貢獻。"
L["Language"] = "語言"
L["Choose the language used by the DragonUI interface."] = "選擇DragonUI介面使用的語言。"
L["Follow the client language"] = "跟隨客戶端語言"

L["Commands: /dragonui, /dui, /pi — /dragonui edit (editor) — /dragonui help"] = "命令：/dragonui, /dui, /pi — /dragonui edit (編輯) — /dragonui help"
L["GitHub (select and Ctrl+C to copy):"] = "GitHub (選中後按Ctrl+C複製)："
L["All"] = "全部"
L["Error:"] = "錯誤："
L["Error: DragonUI addon not found!"] = "錯誤：未找到DragonUI外掛！"

-- ============================================================================
-- 靜態彈出框
-- ============================================================================

L["Changing this setting requires a UI reload to apply correctly."] = "更改此設定需要重新載入介面才能正確應用。"
L["Reload UI"] = "重新載入介面"
L["Not Now"] = "以後再說"
L["Reload Now"] = "立即過載"
L["Cancel"] = "取消"
L["Yes"] = "是"
L["No"] = "否"

-- ============================================================================
-- 標籤頁名稱
-- ============================================================================

L["Modules"] = "模組"
L["Action Bars"] = "動作條"
L["Additional Bars"] = "附加動作條"
L["Minimap"] = "小地圖"
L["Profiles"] = "配置檔案"
L["Unit Frames"] = "單位框架"
L["XP & Rep Bars"] = "經驗值 & 聲望條"
L["Chat"] = "聊天"
L["Appearance"] = "外觀"


-- ============================================================================
-- 模組標籤頁
-- ============================================================================

-- 標題和描述
L["Module Control"] = "模組控制"
L["Enable or disable specific DragonUI modules"] = "啟用或禁用特定的DragonUI模組"
L["Toggle individual modules on or off. Disabled modules revert to the default Blizzard UI."] = "切換單個模組的啟用/禁用。禁用的模組將恢復為預設的暴雪介面。"
L["Visual enhancements that add Dragonflight-style polish to the UI."] = "為介面新增巨龍時代風格潤色的視覺增強功能。"
L["Warning: These are individual module controls. The options above may control multiple modules at once. Changes here will be reflected above and vice versa."] = "警告：這是單個模組的控制。上方的選項可能同時控制多個模組。此處的更改會反映在上方，反之亦然。"
L["Warning:"] = "警告："
L["Individual overrides. The grouped toggles above take priority."] = "單個模組的覆蓋設定。上方的分組開關具有優先順序。"
L["Advanced - Individual Module Control"] = "高階 - 單個模組控制"

-- 分割槽標題
L["Cast Bars"] = "施法條"
L["Other Modules"] = "其他模組"
L["UI Systems"] = "介面系統"
L["Enable All Action Bar Modules"] = "啟用所有動作條模組"
L["Cast Bar"] = "施法條"
L["Custom player, target, and focus cast bars"] = "自定義玩家、目標和焦點的施法條"
L["Cooldown text on action buttons"] = "動作按鈕上的冷卻時間文字"
L["Shaman totem bar positioning and styling"] = "薩滿圖騰條的位置和樣式"
L["Dragonflight-styled player unit frame"] = "巨龍時代風格的玩家單位框架"
L["Dragonflight-styled boss target frames"] = "巨龍時代風格的團隊首領目標框架"

-- 開關標籤
L["Action Bars System"] = "動作條系統"
L["Micro Menu & Bags"] = "微型選單和揹包"
L["Cooldown Timers"] = "冷卻計時器"
L["Minimap System"] = "小地圖系統"
L["Buff Frame System"] = "增益效果框體系統"
L["Dark Mode"] = "暗色模式"
L["Item Quality Borders"] = "物品品質邊框"
L["Enable Enhanced Tooltips"] = "啟用增強型滑鼠提示"
L["KeyBind Mode"] = "按鍵繫結模式"
L["Quest Tracker"] = "任務追蹤器"

-- 模組開關描述
L["Enable DragonUI player castbar. When disabled, shows default Blizzard castbar."] = "啟用DragonUI玩家施法條。禁用時，顯示預設的暴雪施法條。"
L["Enable DragonUI player castbar styling."] = "啟用DragonUI玩家施法條樣式。"
L["Enable DragonUI target castbar. When disabled, shows default Blizzard castbar."] = "啟用DragonUI目標施法條。禁用時，顯示預設的暴雪施法條。"
L["Enable DragonUI target castbar styling."] = "啟用DragonUI目標施法條樣式。"
L["Enable DragonUI focus castbar. When disabled, shows default Blizzard castbar."] = "啟用DragonUI焦點施法條。禁用時，顯示預設的暴雪施法條。"
L["Enable DragonUI focus castbar styling."] = "啟用DragonUI焦點施法條樣式。"
L["Enable the complete DragonUI action bars system. This controls: Main action bars, vehicle interface, stance/shapeshift bars, pet action bars, multicast bars (totems/possess), button styling, and hide Blizzard elements. When disabled, all action bar related features will use default Blizzard interface."] = "啟用完整的DragonUI動作條系統。控制內容包括：主動作條、載具介面、姿態/變形條、寵物動作條、多目標條（圖騰/控制）、按鈕樣式，以及隱藏暴雪元素。禁用時，所有動作條相關功能將使用預設的暴雪介面。"
L["Master toggle for the complete action bars system."] = "動作條系統的總開關。"
L["Includes main bars, vehicle, stance, pet, totem bars, and button styling."] = "包括主動作條、載具、姿態、寵物、圖騰條和按鈕樣式。"
L["Apply DragonUI micro menu and bags system styling and positioning. Includes character button, spellbook, talents, etc. and bag management. When disabled, these elements will use default Blizzard positioning and styling."] = "應用DragonUI微型選單和揹包系統的樣式和位置。包括角色按鈕、法術書、天賦等以及揹包管理。禁用時，這些元素將使用預設的暴雪位置和樣式。"
L["Micro menu and bags styling."] = "微型選單和揹包樣式。"
L["Show cooldown timers on action buttons. When disabled, cooldown timers will be hidden and the system will be completely deactivated."] = "在動作按鈕上顯示冷卻計時器。禁用時，冷卻計時器將被隱藏，系統將完全停用。"
L["Show cooldown timers on action buttons."] = "在動作條上顯示冷卻時間數字。"
L["Enable DragonUI minimap enhancements including custom styling, positioning, tracking icons, and calendar. When disabled, uses default Blizzard minimap appearance and positioning."] = "啟用DragonUI小地圖增強功能，包括自定義樣式、位置、追蹤圖示和日曆。禁用時，使用預設的暴雪小地圖外觀和位置。"
L["Minimap styling, tracking icons, and calendar."] = "小地圖樣式、追蹤圖示和日曆。"
L["Enable DragonUI buff frame with custom styling, positioning, and toggle button functionality. When disabled, uses default Blizzard buff frame appearance and positioning."] = "啟用具有自定義樣式、位置和切換按鈕功能的DragonUI增益效果框體。禁用時，使用預設的暴雪增益效果框體外觀和位置。"
L["Buff frame styling and toggle button."] = "增益效果框體樣式和切換按鈕。"
L["Auras"] = "光環"
L["Show Toggle Button"] = "顯示切換按鈕"
L["Show a collapse/expand button next to the buff icons."] = "在增益圖示旁顯示收合/展開按鈕。"
L["Vanity Buffs"] = true
L["Vanity buffs are cosmetic auras consolidated by Ascension (mounts, toys, transmog effects). The consolidated container is provided by the Ascension_VanityCollection addon."] = true
L["Hide Vanity Buffs"] = true
L["Hide the Ascension vanity-buff consolidated container and its contained auras from the buff frame. Existing buff order and positioning are preserved."] = true
L["Separate Weapon Enchants"] = "分離武器附魔"
L["Detach weapon enchant icons (poisons, sharpening stones, etc.) from the buff bar into their own independently moveable frame. Position it freely using Editor Mode."] = "將武器附魔圖示（毒藥、磨刀石等）從增益效果條分離到它們自己獨立可移動的框體中。使用編輯模式自由放置。"
L["Weapon Enchants"] = "武器附魔"
L["Weapon enchant icons include rogue poisons, sharpening stones, wizard oils, and similar temporary weapon enhancements."] = "武器附魔圖示包括：盜賊毒藥、磨刀石、巫師之油及其他類似的臨時武器強化效果。"
L["When enabled, a 'Weapon Enchants' mover appears in Editor Mode that you can drag to any position on screen."] = "啟用後，編輯模式中會出現“武器附魔”移動器，可將其拖拽到螢幕任何位置。"
L["Positions"] = "位置"
L["Player Buffs & Debuffs"] = "玩家增益與減益"
L["Layout settings for the player buff and debuff bar. These do not affect target or focus auras."] = "玩家增益與減益列的版面設定。不影響目標或焦點光環。"
L["Buff Order"] = "增益排序"
L["Default (Blizzard)"] = "預設（暴雪）"
L["How to sort player buff icons on the buff bar."] = "玩家增益列圖示的排序方式。"
L["Player Buffs First"] = "自身增益優先"
L["Other Player Buffs First"] = "他人增益優先"
L["Duration Buffs First"] = "按時長優先"
L["Buffs Per Row"] = "每行增益數"
L["Debuffs Per Row"] = "每行減益數"
L["How many buff icons to show in each row."] = "每行顯示多少個增益圖示。"
L["How many debuff icons to show in each row."] = "每行顯示多少個減益圖示。"
L["Max Buff Rows"] = "最大增益行數"
L["Max Debuff Rows"] = "最大減益行數"
L["Maximum number of buff rows to display. Use 0 for no limit."] = "最多顯示多少行增益。0 表示不限制。"
L["Maximum number of debuff rows to display. Use 0 for no limit."] = "最多顯示多少行減益。0 表示不限制。"
L["Buff Horizontal Gap"] = "增益水平間距"
L["Debuff Horizontal Gap"] = "減益水平間距"
L["Buff Vertical Gap"] = "增益垂直間距"
L["Debuff Vertical Gap"] = "減益垂直間距"
L["Space between buff rows."] = "增益行之間的間距。"
L["Space between debuff rows."] = "減益行之間的間距。"
L["Debuff Attached Offset Y"] = "減益附著偏移 Y"
L["Vertical gap below the buff bar when debuffs are attached (not detached in Editor Mode)."] = "減益附著在增益列下方時的垂直間距（未在編輯模式中分離時）。"
L["Layout Preview"] = "版面預覽"
L["Shows fake buff and debuff icons so you can tune scale, rows, and spacing without needing real auras. Turn this off when finished."] = "顯示假的增益與減益圖示，讓你不需要真實光環就能調整縮放、列數與間距。完成後請關閉。"
L["Enable Layout Preview"] = "啟用版面預覽"
L["Preview Buff Count"] = "預覽增益數量"
L["Preview Debuff Count"] = "預覽減益數量"
L["Reset Buff Frame Position"] = "重置增益效果框體位置"
L["Reset Weapon Enchant Position"] = "重置武器附魔位置"
L["Buff frame position reset."] = "增益效果框體位置已重置。"
L["Weapon enchant position reset."] = "武器附魔位置已重置。"
L["Target & Focus Aura Customization"] = "目標與焦點光環自訂"
L["Customize target/focus aura icons and timers."] = "自訂目標與焦點光環的圖示與計時文字。"
L["Aura Timers"] = "光環計時器"
L["Show aura timers on Target and Focus independently."] = "分別在目標與焦點上獨立顯示光環計時器。"
L["Aura Icon Customization"] = "光環圖示自訂"
L["Customize icon size, scale, and stack text for target/focus auras."] = "自訂目標/焦點光環圖示的大小、縮放與層數文字。"
L["Customize Aura Icons"] = "自訂光環圖示"
L["Enable custom icon styling for target/focus aura icons."] = "為目標/焦點光環圖示啟用自訂樣式。"
L["Target Aura Timer Settings"] = "目標光環計時設定"
L["Enable Target Aura Timers"] = "啟用目標光環計時器"
L["Target Aura Timer Size"] = "目標光環計時器大小"
L["Target Aura Minimum Duration (Seconds)"] = "目標光環最短持續時間（秒）"
L["Target Aura Maximum Duration (Minutes)"] = "目標光環最長持續時間（分鐘）"
L["Ignore Keeper's Aura from Target"] = "忽略目標的守護者光環"
L["Hide all buffs on the target whose name starts with 'Keeper's'."] = "隱藏目標名稱以「Keeper's」開始的所有增益效果。"
L["Focus Aura Timer Settings"] = "焦點光環計時設定"
L["Enable Focus Aura Timers"] = "啟用焦點光環計時器"
L["Focus Aura Timer Size"] = "焦點光環計時器大小"
L["Focus Aura Minimum Duration (Seconds)"] = "焦點光環最短持續時間（秒）"
L["Focus Aura Maximum Duration (Minutes)"] = "焦點光環最長持續時間（分鐘）"
L["Only show aura timers when remaining duration is above this value (seconds)."] = "僅在剩餘持續時間高於此值時顯示光環計時器（秒）。"
L["Only show aura timers when remaining duration is below this value (minutes). Use 0 to disable this limit."] = "僅在剩餘持續時間低於此值時顯示光環計時器（分鐘）。使用 0 可停用此限制。"
L["Timer Text Settings"] = "計時文字設定"
L["Aura Buffs"] = "光環增益"
L["Aura Debuffs"] = "光環減益"
L["Duration Text Anchor"] = "持續時間文字錨點"
L["Duration X Offset"] = "持續時間X偏移"
L["Duration Y Offset"] = "持續時間Y偏移"
L["Stack Text Settings"] = "層數文字設定"
L["Stack Text Anchor"] = "層數文字錨點"
L["Stack X Offset"] = "層數X偏移"
L["Stack Y Offset"] = "層數Y偏移"
L["Duration Font"] = "持續時間字型"
L["Stack Font"] = "層數字型"
L["Actionbar Font"] = "動作列字型"
L["Primary Font"] = "主要字型"
L["Narrow Font"] = "窄體字型"
L["Arial Font"] = "Arial字型"
L["System Font"] = "系統字型"
L["Aura Size"] = "光環大小"
L["Auras Per Row"] = "每行光環數"
L["Discrete size steps: how many auras fit in a full-width row. 6 is the Blizzard default (17px). Your own auras render slightly larger, and rows beside a visible Target-of-Target are narrower, so fewer may fit there."] = "離散尺寸檔位：一整行可容納多少個光環。6 為暴雪預設值（17px）。你自己施放的光環會略大一些；目標的目標可見時，其旁邊的行更窄，可容納的光環更少。"
L["Buff Icon Size"] = "增益圖示大小"
L["Buff Icon Scale"] = "增益圖示縮放"
L["Buff Stack Font Size"] = "增益層數字型大小"
L["Debuff Icon Size"] = "減益圖示大小"
L["Debuff Icon Scale"] = "減益圖示縮放"
L["Debuff Stack Font Size"] = "減益層數字型大小"
L["Show Debuff Cooldown Swipe"] = "顯示減益冷卻掃描"
L["Also show a radial cooldown sweep on each debuff icon."] = "在每個減益圖示上額外顯示放射狀冷卻掃描效果。"
L["Debuff Horizontal Offset"] = "減益水平偏移"
L["Shifts the debuff icon row left or right relative to its default position."] = "相對預設位置左右移動減益圖示列。"
L["Debuff Vertical Offset"] = "減益垂直偏移"
L["Shifts the debuff icon row up or down relative to its default position."] = "相對預設位置上下移動減益圖示列。"
L["Show Debuff Position Debug Box"] = "顯示減益位置除錯框"
L["Displays a box showing where the debuff icon row starts and ends, even when no debuffs are active."] = "顯示一個方框，標示減益圖示列的起止位置，即使目前沒有生效的減益效果。"
L["Debuff Cooldown Swipe Style"] = "減益冷卻掃描樣式"
L["Choose the visual style of the cooldown sweep. These texture-based styles stay aligned while the nameplate is moving."] = "選擇冷卻掃描的視覺樣式。這些基於材質的樣式在姓名板移動時仍會保持對齊。"
L["Shade Fill"] = "陰影填充"
L["Quadrant Sweep"] = "象限掃描"
L["Square Radial Sweep"] = "方形放射掃描"
L["Reset Aura Customization"] = "重置光環自訂"
L["Reset Aura Timers"] = "重置光環計時器"
L["Aura timer settings reset."] = "光環計時器設定已重置。"
L["Aura icon customization settings reset."] = "光環圖示自訂設定已重置。"

L["DragonUI quest tracker positioning and styling."] = "DragonUI任務追蹤器的位置和樣式設定。"
L["LibKeyBound integration for intuitive hover + key press binding."] = "滑鼠懸停+按鍵繫結的直觀按鍵繫結（LibKeyBound）功能。"

-- 切換按鍵繫結模式描述
L["Toggle keybinding mode. Hover over action buttons and press keys to bind them instantly. Press ESC to clear bindings."] = "切換按鍵繫結模式。將滑鼠懸停在動作按鈕上並按鍵盤按鍵即可立即繫結。按ESC鍵清除繫結。"

-- 啟用/禁用動態描述
L["Enable/disable "] = "啟用/禁用："

-- 暗色模式
L["Dark Mode Intensity"] = "暗色模式強度"
L["Light (subtle)"] = "淺（輕微）"
L["Medium (balanced)"] = "中（均衡）"
L["Dark (maximum)"] = "深（最大）"
L["Apply darker tinted textures to all UI chrome: action bars, unit frames, minimap, bags, micro menu, and more."] = "對所有介面裝飾元素應用深色紋理：動作條、單位框架、小地圖、揹包、微型選單等。"
L["Apply darker tinted textures to all UI elements."] = "對所有介面元素應用深色著色。"
L["Darkens UI borders and chrome only: action bar borders, unit frame borders, minimap border, bag slot borders, micro menu, castbar borders, and decorative elements. Icons, portraits, and abilities are never affected."] = "僅使介面邊框和裝飾變暗：動作條邊框、單位框架邊框、小地圖邊框、揹包格子邊框、微型選單、施法條邊框和裝飾元素。圖示、頭像和技能不受影響。"
L["Enable Dark Mode"] = "啟用暗色模式"

-- 暗色模式 - 自定義顏色
L["Custom Color"] = "自定義顏色"
L["Override presets with a custom tint color."] = "使用自定義色調顏色覆蓋預設。"
L["Tint Color"] = "色調選擇"
L["Intensity"] = "明暗濃度"

-- 距離指示器
L["Tint action button icons when target is out of range (red), not enough mana (blue), or unusable (gray)."] = "當目標超出範圍（紅色）、法力不足（藍色）或無法使用時（灰色），為動作按鈕圖示著色。"
L["Tints action button icons based on range and usability: red = out of range, blue = not enough mana, gray = unusable."] = "根據距離和使用情況為動作按鈕圖示著色：紅色=超出範圍，藍色=法力不足，灰色=不可用。"
L["Enable Range Indicator"] = "啟用距離指示器"
L["Color action button icons when target is out of range or ability is unusable."] = "當目標超出範圍或技能無法使用時，為動作條圖示著色。"

-- 按鍵觸發
L["Key Press"] = "按鍵觸發"
L["Fires action bar abilities the instant you press a key instead of when you release it, shaving reaction-time latency. Most useful for interrupts, dispels, and PvP."] = "在按下按鍵時立即觸發動作條技能，而非放開時，減少反應延遲。非常適合打斷、驅散和PvP。"
L["Enable Key Press"] = "啟用按鍵觸發"
L["Fire abilities on key press instead of key release."] = "按下按鍵時觸發技能，而非放開時。"

-- 物品品質邊框
L["Show colored glow borders on action buttons containing items, colored by item quality (green = uncommon, blue = rare, purple = epic, etc.)."] = "在包含物品的動作按鈕上顯示按物品品質著色的發光邊框（綠色=優秀，藍色=精良，紫色=史詩等）。"
L["Enable Item Quality Borders"] = "啟用物品品質邊框"
L["Show quality-colored borders on items in bags, character panel, bank, merchant, and inspect frames."] = "在揹包、角色面板、銀行、商人視窗和觀察視窗中的物品上顯示按品質著色的邊框。"
L["Adds quality-colored glow borders to items in your bags, character panel, bank, merchant, and inspect frames: green = uncommon, blue = rare, purple = epic, orange = legendary."] = "為揹包、角色面板、銀行、商人和觀察視窗中的物品新增按品質著色的發光邊框：綠色=優秀，藍色=精良，紫色=史詩，橙色=傳說。"
L["Minimum Quality"] = "最低品質"
L["Only show colored borders for items at or above this quality level."] = "僅為此品質等級及以上的物品顯示彩色邊框。"
L["Poor"] = "粗糙"
L["Common"] = "普通"
L["Uncommon"] = "優秀"
L["Rare"] = "精良"
L["Epic"] = "史詩"
L["Legendary"] = "傳說"

-- 增強型滑鼠提示
L["Enhanced Tooltips"] = "增強型滑鼠提示"
L["Improves GameTooltip with class-colored borders, class-colored names, target-of-target info, and styled health bars."] = "改進遊戲內滑鼠提示：職業顏色邊框、職業顏色名稱、目標的目標資訊以及樣式化的生命條。"
L["Activate all tooltip improvements. Sub-options below control individual features."] = "啟用所有滑鼠提示改進功能。下方子選項控制單個功能。"
L["Class-Colored Border"] = "職業顏色邊框"
L["Color the tooltip border by the unit's class (players) or reaction (NPCs)."] = "根據單位職業（玩家）或關係（NPC）為滑鼠提示邊框著色。"
L["Class-Colored Name"] = "職業顏色名稱"
L["Color the unit name text in the tooltip by class color (players only)."] = "在滑鼠提示中，根據職業顏色顯示單位名稱文字（僅限玩家）。"
L["Add a 'Targeting: <name>' line showing who the unit is targeting."] = "新增一行“目標：<名字>”以顯示該單位的目標是誰。"
L["Add a 'Targeting: <name>' line to the tooltip showing who the unit is targeting."] = "在滑鼠提示中新增一行“目標：<名字>”，顯示該單位的目標。"
L["Styled Health Bar"] = "樣式化生命條"
L["Restyle the tooltip health bar with class/reaction colors."] = "使用職業/關係顏色重新設計滑鼠提示生命條樣式。"
L["Restyle the tooltip health bar with class/reaction colors and slimmer look."] = "使用職業/關係顏色和更纖細的外觀重新設計滑鼠提示生命條。"
L["Anchor to Cursor"] = "錨定到游標"
L["Make the tooltip follow the cursor position instead of the default anchor."] = "使滑鼠提示跟隨游標位置，而不是預設的錨點。"
L["Show Aura Source"] = "顯示光環來源"
L["Show the caster's name (class-colored) and spell ID on buff and debuff tooltips."] = "在增益/減益提示中顯示施放者的職業色名稱與法術 ID。"

-- 聊天修改
L["Enable Chat Mods"] = "啟用聊天功能修改"
L["Enables or disables Chat Mods."] = "啟用或停用聊天功能修改。"
L["Show Link Tooltips on Hover"] = "滑鼠移上連結時顯示提示"
L["Show tooltips for supported chat links without holding Alt."] = "無需按住 Alt 鍵即可顯示支援的聊天連結提示。"
L["Editbox Position"] = "輸入框位置"
L["Choose where the chat editbox is positioned."] = "選擇聊天輸入框的擺放位置。"
L["Top"] = "頂部"
L["Bottom"] = "底部"
L["Left"] = "左側"
L["Right"] = "右側"
L["Middle"] = "中間"
L["Tab & Button Fade"] = "標籤與按鈕淡出"
L["How visible chat tabs are when not hovered. 0 = fully hidden, 1 = fully visible."] = "未懸停時聊天分頁的可見度。0 = 完全隱藏，1 = 完全可見。"
L["Chat Style Opacity"] = "聊天樣式透明度"
L["Minimum opacity of the custom chat background. At 0 it fades with tabs; above 0 it stays partially visible when idle."] = "自訂聊天背景的最小透明度。0時與標籤同步淡出；超出0則閒置時仍部分可見。"
L["Text Box Min Opacity"] = "輸入框最小透明度"
L["Minimum opacity of the text input box when idle. At 0 it fades with tabs; above 0 it stays partially visible."] = "閒置時文字輸入框的最小透明度。0時與標籤同步淡出；超出0則仍部分可見。"
L["Chat Style"] = "聊天樣式"
L["Visual style for the chat frame background."] = "聊天框背景的視覺樣式。"
L["Editbox Style"] = "輸入框樣式"
L["Visual style for the chat input box background."] = "聊天輸入框背景的視覺樣式。"
L["Vanilla Editbox"] = "原版輸入框"
L["Use the default chat input appearance."] = "使用預設聊天輸入框外觀。"
L["Dark"] = "暗色"
L["DragonUI Style"] = "DragonUI 樣式"
L["Nocturne"] = "夜曲"

L["Recommended Addon"] = "推薦插件"
L["For a smoother, more polished chat experience, check out CleanerChat-WotLK — it adds chat filter tweaks, improved chat styling, and Glass UI overlays."] = "想要更流暢的聊天體驗，試試 CleanerChat-WotLK — 提供聊天篩選、改良樣式和 Glass UI 疊層。"
L["Download (select and Ctrl+C to copy):"] = "下載（選取後 Ctrl+C 複製）："


-- 揹包整合 (Bagster)
L["Enable Bagster"] = "啟用揹包整合 (Bagster)"
L["All-in-one bag replacement with item filtering, search, quality indicators, and bank integration."] = "具有物品過濾、搜尋、品質指示器和銀行整合功能的一體化揹包替代外掛。"
L["Bagster Settings"] = "Bagster設定"

-- Item usability tint
L["Item Usability"] = "物品可用性"
L["Tint Unusable Items"] = "標紅不可用物品"
L["Color icons red for gear and usable items your character cannot equip or use (wrong armor type, level, class, etc.)."] = "將角色無法裝備或使用的裝備和可用物品圖示標為紅色（錯誤的護甲類型、等級、職業等）。"

-- 揹包整理
L["Bag Sort"] = "揹包整理"
L["Enable Bag Sort"] = "啟用揹包整理"
L["Sort buttons for bags and bank. Sorts items by type, rarity, level, and name."] = "揹包和銀行的整理按鈕。按型別、稀有度、等級和名稱排序物品。"
L["Add sort buttons to bag and bank frames. Also enables /sort and /sortbank slash commands."] = "為揹包和銀行視窗新增整理按鈕。同時啟用 /sort 和 /sortbank 命令。"
L["Fill Bank Stacks from Bags"] = "以揹包物品補滿銀行堆疊"
L["Pull matching items from your bags into partial bank stacks when sorting the bank."] = "整理銀行時，先從揹包取出相同物品補滿銀行中的未滿堆疊。"
L["Sort bags and bank items with buttons"] = "透過按鈕整理揹包和銀行物品"
L["Lock Icon Color"] = "鎖定圖示顏色"
L["Color used to tint the padlock icon shown on locked bag/bank slots."] = "用於為揹包/銀行鎖定格子上顯示的鎖頭圖示上色的顏色。"
L["Reverse Stack Order"] = "反向堆疊"
L["Stack sorted items from the end of each bag so empty slots stay at the top."] = "從每個背包的末尾堆疊已排序的物品，使空位保持在頂部。"

L["Show 'All' Tab"] = "顯示“全部”標籤頁"
L["Show the 'All' category tab that displays all items without filtering."] = "顯示不過濾、展示所有物品的“全部”分類標籤頁。"
L["Equipment"] = "裝備"
L["Usable"] = "消耗品"
L["Show Equipment Tab"] = "顯示裝備標籤頁"
L["Show the Equipment category tab for armor and weapons."] = "顯示護甲和武器的裝備分類標籤頁。"
L["Show Usable Tab"] = "顯示消耗品標籤頁"
L["Show the Usable category tab for consumables and devices."] = "顯示消耗品和裝置的“消耗品”分類標籤頁。"
L["Show Consumable Tab"] = "顯示消耗品標籤頁"
L["Show the Consumable category tab."] = "顯示消耗品分類標籤頁。"
L["Show Quest Tab"] = "顯示任務標籤頁"
L["Show the Quest items category tab."] = "顯示任務物品分類標籤頁。"
L["Show Trade Goods Tab"] = "顯示商品標籤頁"
L["Show the Trade Goods category tab (includes gems and recipes)."] = "顯示商品（包括寶石和配方）分類標籤頁。"
L["Show Miscellaneous Tab"] = "顯示其他標籤頁"
L["Show the Miscellaneous items category tab."] = "顯示其他物品分類標籤頁。"
L["Left Side Tabs"] = "左側標籤頁"
L["Place category filter tabs on the left side of the bag frame instead of the right."] = "將分類過濾標籤頁放在揹包視窗的左側而非右側。"
L["Place category filter tabs on the left side of the bank frame instead of the right."] = "將分類過濾標籤頁放在銀行視窗的左側而非右側。"
L["Changes require closing and reopening bags to take effect."] = "更改需要關閉並重新開啟揹包才能生效。"
L["Subtabs"] = "子標籤頁"
L["Configure which bottom subtabs appear within each category tab. Applies to both inventory and bank."] = "配置每個分類標籤頁內顯示哪些底部的子標籤頁。同時適用於揹包和銀行。"
L["Normal"] = "普通"
L["Trade Bags"] = "專業揹包"
L["Show the Normal bags subtab (non-profession bags)."] = "顯示普通揹包（非專業揹包）子標籤頁。"
L["Show the Trade bags subtab (profession bags)."] = "顯示專業揹包子標籤頁。"
L["Show the Armor subtab."] = "顯示護甲子標籤頁。"
L["Show the Weapon subtab."] = "顯示武器子標籤頁。"
L["Show the Trinket subtab."] = "顯示飾品子標籤頁。"
L["Show the Consumable subtab."] = "顯示消耗品子標籤頁。"
L["Show the Devices subtab."] = "顯示裝置子標籤頁。"
L["Show the Trade Goods subtab."] = "顯示商品子標籤頁。"
L["Show the Gem subtab."] = "顯示寶石子標籤頁。"
L["Show the Recipe subtab."] = "顯示配方子標籤頁。"
L["Configure Bagster bag replacement settings."] = "配置揹包整合 (Bagster) 替代設定。"
L["Category Tabs"] = "分類標籤頁"
L["Inventory Tabs"] = "揹包標籤頁"
L["Bank Tabs"] = "銀行標籤頁"
L["Inventory"] = "揹包"
L["Bank"] = "銀行"
L["Choose which category tabs appear on the bag frame. Changes require closing and reopening bags to take effect."] = "選擇在揹包視窗上顯示哪些分類標籤頁。更改需要關閉並重新開啟揹包才能生效。"
L["Choose which category tabs appear on the inventory bag frame."] = "選擇在揹包視窗上顯示哪些分類標籤頁。"
L["Choose which category tabs appear on the bank frame."] = "選擇在銀行視窗上顯示哪些分類標籤頁。"
L["Gold Display"] = "金幣顯示"
L["Text Only"] = "僅文字"
L["Gold Icons"] = "金幣圖標"
L["(disabled by Colorblind Mode)"] = "(已被色盲模式停用)"
L["Item Scale"] = "物品縮放"
L["Maximum size of item slots. The grid still shrinks them to fit the frame."] = "物品格子的最大尺寸。網格會縮小它們以適應視窗。"
L["Item Spacing"] = "物品間距"
L["Gap between item slots in the grid."] = "網格中物品格子之間的間距。"
L["Quality Glow"] = "品質光環"
L["Show a colored ring on uncommon and better items."] = "在優秀及以上品質的物品上顯示彩色邊框。"
L["Quest Item Glow"] = "任務物品光環"
L["Highlight quest items with a golden border."] = "用金色邊框突出顯示任務物品。"
L["Glow Intensity"] = "光環強度"
L["Opacity of the quality ring on item slots."] = "物品格子品質邊框的不透明度。"
L["Quality Filter Row"] = "品質篩選行"
L["Show the rarity filter dots at the bottom of the bag frame."] = "在背包視窗底部顯示稀有度篩選點。"

-- 高階模組 - 備用顯示名稱
L["Main Bars"] = "主動作條"
L["Vehicle"] = "載具"
L["Multicast"] = "多目標施法"
L["Buttons"] = "按鈕"
L["Hide Blizzard Elements"] = "隱藏暴雪預設元素"
L["Buffs"] = "增益效果"
L["KeyBinding"] = "按鍵繫結"
L["Cooldowns"] = "冷卻時間"

-- 高階模組 - RegisterModule 顯示名稱（來自模組檔案）
L["Micro Menu"] = "微型選單"
L["Loot Roll"] = "擲骰"
L["Key Binding"] = "按鍵繫結"
L["Item Quality"] = "物品品質"
L["Hide Blizzard"] = "隱藏原始介面"
L["Tooltip"] = "滑鼠提示"

-- 高階模組 - RegisterModule 描述（來自模組檔案）
L["Micro menu and bags system styling and positioning"] = "微型選單和揹包系統的樣式/位置設定"
L["Quest tracker positioning and styling"] = "任務追蹤器位置和樣式設定"
L["Enhanced tooltip styling with class colors and health bars"] = "帶有職業顏色和生命條的增強型滑鼠提示樣式"
L["Hide default Blizzard UI elements"] = "隱藏預設的暴雪介面元素"
L["Custom minimap styling, positioning, tracking icons and calendar"] = "自定義小地圖樣式、位置、追蹤圖示和日曆"
L["Main action bars, status bars, scaling and positioning"] = "主動作條、狀態條、縮放和位置"
L["LibKeyBound integration for intuitive keybinding"] = "用於直觀按鍵繫結的LibKeyBound整合"
L["Color item borders by quality in bags, character panel, bank, and merchant"] = "在揹包、角色面板、銀行、商人視窗按物品品質為邊框著色"
L["Darken UI borders and chrome"] = "使介面邊框和裝飾變暗"
L["Action button styling and enhancements"] = "動作按鈕樣式和增強功能"
L["Vehicle interface enhancements"] = "載具介面增強功能"
L["Stance/shapeshift bar positioning and styling"] = "姿態/變形條位置和樣式設定"
L["Pet action bar positioning and styling"] = "寵物動作條位置和樣式設定"
L["Multicast (totem/possess) bar positioning and styling"] = "多目標施法（圖騰/控制）條位置和樣式設定"
L["Chat Mods"] = "聊天功能修改"
L["Chat enhancements: hide buttons, editbox position, URL copy, chat copy, link hover, tell target"] = "聊天增強：隱藏按鈕、輸入框位置、URL/聊天覆制、連結懸停提示、/告訴目標"
L["Bagster"] = "揹包整合 (Bagster)"
L["All-in-one bag replacement with filtering and search"] = "具有過濾和搜尋功能的一體化揹包替代"

-- ============================================================================
-- 動作條標籤頁
-- ============================================================================

-- 子標籤頁
L["Visibility"] = "可見性"

-- 縮放比例部分
L["Action Bar Scales"] = "動作條縮放比例"
L["Main Bar Scale"] = "主動作條縮放"
L["Right Bar Scale"] = "右側動作條縮放"
L["Left Bar Scale"] = "左側動作條縮放"
L["Bottom Left Bar Scale"] = "左下動作條縮放"
L["Bottom Right Bar Scale"] = "右下動作條縮放"
L["Scale for main action bar"] = "主動作條縮放"
L["Scale for right action bar (MultiBarRight)"] = "右側動作條縮放 (MultiBarRight)"
L["Scale for left action bar (MultiBarLeft)"] = "左側動作條縮放 (MultiBarLeft)"
L["Scale for bottom left action bar (MultiBarBottomLeft)"] = "左下動作條縮放 (MultiBarBottomLeft)"
L["Scale for bottom right action bar (MultiBarBottomRight)"] = "右下動作條縮放 (MultiBarBottomRight)"
L["Reset All Scales"] = "重置所有縮放比例"
L["Reset all action bar scales to their default values (0.9)"] = "將所有動作條縮放比例重置為其預設值(0.9)"
L["All action bar scales reset to default values (0.9)"] = "所有動作條縮放比例已重置為預設值(0.9)"
L["All action bar scales reset to 0.9"] = "所有動作條縮放比例重置為0.9"

-- 位置部分
L["Action Bar Positions"] = "動作條位置"
L["Tip: Use the Move UI Elements button above to reposition action bars with your mouse."] = "提示：使用上方的“移動介面元素”按鈕，用滑鼠重新定位動作條。"
L["Left Bar Horizontal"] = "左側條水平放置"
L["Make the left secondary bar horizontal instead of vertical."] = "將左側輔助動作條改為水平放置，而非垂直。"
L["Right Bar Horizontal"] = "右側條水平放置"
L["Make the right secondary bar horizontal instead of vertical."] = "將右側輔助動作條改為水平放置，而非垂直。"

-- 按鈕外觀部分
L["Button Appearance"] = "按鈕外觀"
L["Hide Secondary Bar Background"] = "隱藏次要動作條背景"
L["Hide the background of secondary bar buttons (stance, pet, totem, etc.)."] = "隱藏次要動作條按鈕的背景（姿態、寵物、圖騰等）。"
L["Hide Main Bar Button Background"] = "隱藏主動作條按鈕背景"
L["Hide the background of main action bar buttons."] = "隱藏主動作條按鈕的背景。"
L["Hide Main Bar Background"] = "隱藏主動作條背景"
L["Hide the background texture of the main action bar (makes it completely transparent)"] = "隱藏主動作條的背景紋理（使其完全透明）"
L["Hide the background texture of the main action bar."] = "隱藏主動作條的背景紋理。"

-- 行為
L["Behavior"] = "行為"
L["Disable Form Page Switching"] = "禁用形態頁面切換"
L["When enabled, the main action bar stays on the same page regardless of stance/form changes. Useful for druids, warriors, and CoA Classes who prefer manual page management."] = "啟用後，主動作條將保持在當前頁面，不受姿態/形態變化影響。適合偏好手動管理頁面的德魯伊、戰士和CoA職業。"

-- 文字可見性
L["Text Visibility"] = "文字可見性"
L["Count Text"] = "數量文字"
L["Show Count"] = "顯示數量"
L["Show Count Text"] = "顯示疊加次數文字"
L["Hotkey Text"] = "按鍵文字"
L["Show Hotkey"] = "顯示按鍵"
L["Show Hotkey Text"] = "顯示按鍵文字"
L["Show the button frame on pet slots with no ability assigned."] = "在未指定技能的寵物欄位上顯示按鈕邊框。"
L["Range Indicator"] = "距離指示器"
L["Show small range indicator point on buttons"] = "在按鈕上顯示小的距離指示點"
L["Show range indicator dot on buttons."] = "在按鈕上顯示距離指示點。"
L["Macro Text"] = "宏文字"
L["Show Macro Names"] = "顯示宏名稱"
L["Page Numbers"] = "頁面編號"
L["Show Pages"] = "顯示頁面"
L["Show Page Numbers"] = "顯示頁面編號"

-- 冷卻文字
L["Cooldown Text"] = "冷卻文字"
L["Min Duration"] = "最短持續時間"
L["Minimum duration for text triggering"] = "觸發文字顯示的最短持續時間"
L["Minimum duration for cooldown text to appear."] = "冷卻文字出現的最短持續時間設定。"
L["Text Color"] = "文字顏色"
L["Cooldown Text Color"] = "冷卻文字顏色"
L["Size of cooldown text."] = "冷卻文字的大小。"

-- 顏色
L["Colors"] = "顏色"
L["Macro Text Color"] = "宏文字顏色"
L["Color for macro text"] = "宏文字的顏色"
L["Hotkey Text Color"] = "按鍵文字顏色"
L["Hotkey Font Size"] = "按鍵字型大小"
L["Hotkey Shadow Color"] = "按鍵陰影顏色"
L["Shadow color for hotkey text"] = "按鍵文字的陰影顏色"
L["Border Color"] = "邊框顏色"
L["Border color for buttons"] = "按鈕的邊框顏色"

-- 獅鷲裝飾
L["Gryphons"] = "獅鷲裝飾"
L["Gryphon Style"] = "獅鷲樣式"
L["Display style for the action bar end-cap gryphons."] = "動作條兩端的獅鷲裝飾的顯示樣式。"
L["End-cap ornaments flanking the main action bar."] = "主動作條兩端的裝飾紋樣。"
L["Gryphon previews are hidden while D3D9Ex is active to avoid client crashes."] = "啟用 D3D9Ex 時會隱藏獅鷲預覽，以避免客戶端當機。"
L["Style"] = "樣式"
L["Gryphon Scale"] = "獅鷲縮放"
L["Gryphon Offset X"] = "獅鷲水平偏移"
L["Gryphon Offset Y"] = "獅鷲垂直偏移"
L["Moves both gryphons inward or outward, mirrored left/right."] = "將兩側獅鷲向內或向外移動，左右鏡像對稱。"
L["Old"] = "舊樣式"
L["New"] = "新樣式"
L["Flying"] = "飛行"
L["Hide Gryphons"] = "隱藏獅鷲"
L["Classic"] = "經典"
L["Dragonflight"] = "巨龍時代"
L["Hidden"] = "隱藏"
L["Dragonflight (Wyvern)"] = "巨龍時代 (雙足飛龍)"
L["Dragonflight (Gryphon)"] = "巨龍時代 (獅鷲)"

-- 佈區域性分
L["Main Bar Layout"] = "主動作條佈局"
L["Bottom Left Bar Layout"] = "左下動作條佈局"
L["Bottom Right Bar Layout"] = "右下動作條佈局"
L["Right Bar Layout"] = "右側動作條佈局"
L["Left Bar Layout"] = "左側動作條佈局"
L["Configure the main action bar grid layout. Rows are determined automatically from columns and buttons shown."] = "配置主動作條網格佈局。行數根據列數和顯示的按鈕數自動確定。"
L["Columns"] = "列數"
L["Buttons Shown"] = "顯示的按鈕數"
L["Change Button Order"] = "變更按鈕順序"
L["Button Order"] = "按鈕順序"
L["Quick Presets"] = "快速預設"
L["Apply layout presets to multiple bars at once."] = "將佈局預設同時應用於多個動作條。"
L["Both 1x12"] = "均為 1x12"
L["Both 2x6"] = "均為 2x6"
L["Reset All"] = "全部重置"
L["All bar layouts reset to defaults."] = "所有動作條佈局已重置為預設值。"

-- 可見性部分
L["Bar Visibility"] = "動作條可見性"
L["Control when action bars are visible. Bars can show only on hover, only in combat, or both. When no option is checked the bar is always visible."] = "控制動作條的顯示條件。動作條可以僅在滑鼠懸停時顯示、僅在戰鬥中顯示，或兩者皆可。未選擇任何選項時，動作條始終可見。"
L["Enable / Disable Bars"] = "啟用 / 禁用動作條"
L["Bottom Left Bar"] = "左下動作條"
L["Bottom Right Bar"] = "右下動作條"
L["Right Bar"] = "右側動作條"
L["Left Bar"] = "左側動作條"
L["Main Bar"] = "主動作條"
L["Always Hidden"] = "始終隱藏"
L["Show on Hover Only"] = "僅滑鼠懸停時顯示"
L["Show in Combat Only"] = "僅戰鬥中顯示"
L["Hide the main bar until you hover over it."] = "僅在滑鼠懸停時顯示主動作條。"
L["Hide the main bar until you enter combat."] = "僅在進入戰鬥時顯示主動作條。"

-- ============================================================================
-- 附加動作條標籤頁
-- ============================================================================

L["Bars that appear based on your class and situation."] = "根據你的職業和情況出現的動作條。"
L["Specialized bars that appear when needed (stance/pet/vehicle/totems)"] = "需要時出現的特殊動作條（姿態/寵物/載具/圖騰）"
L["Auto-show bars: Stance (Warriors/Druids/DKs) • Pet (Hunters/Warlocks/DKs) • Vehicle (All classes) • Totem (Shamans)"] = "自動顯示的動作條：姿態（戰士/德魯伊/死亡騎士）• 寵物（獵人/術士/死亡騎士）• 載具（全職業）• 圖騰（薩滿）"

-- 通用設定
L["Common Settings"] = "通用設定"
L["Button Size"] = "按鈕大小"
L["Size of buttons for all additional bars"] = "所有附加動作條的按鈕大小"
L["Button Spacing"] = "按鈕間距"
L["Space between buttons for all additional bars"] = "所有附加動作條的按鈕間距"

-- 姿態條
L["Stance Bar"] = "姿態條"
L["Warriors, Druids, Death Knights"] = "戰士、德魯伊、死亡騎士"
L["X Position"] = "水平位置"
L["Y Position"] = "垂直位置"
L["Y Offset"] = "Y軸偏移"
L["Horizontal position of stance bar from screen center. Negative values move left, positive values move right."] = "姿態條相對於螢幕中心的水平位置。負值向左移動，正值向右移動。"

-- 寵物動作條
L["Pet Bar"] = "寵物動作條"
L["Pet Bar Layout"] = "寵物動作條佈局"
L["Hunters, Warlocks, Death Knights - Use editor mode to move"] = "獵人、術士、死亡騎士 - 使用編輯模式移動"
L["Show Empty Slots"] = "顯示空技能槽"
L["Display empty action slots on pet bar"] = "在寵物動作條上顯示空的技能槽"

-- 載具動作條
L["Vehicle Bar"] = "載具動作條"
L["All classes (vehicles/special mounts)"] = "全職業（載具/特殊坐騎）"
L["Custom Art Style"] = "自定義藝術風格"
L["Use custom vehicle bar art style with health/power bars and themed skin. Requires UI reload to apply."] = "使用帶有生命值/能量條和主題皮膚的自定義載具條藝術風格。需要重新載入介面才能應用。"
L["Blizzard Art Style"] = "暴雪藝術風格"
L["Use Blizzard vehicle bar art with health/power display. Requires reload."] = "使用帶有生命值/能量顯示的暴雪預設載具條藝術風格（需要過載介面）。"

-- 圖騰條
L["Totem Bar"] = "圖騰條"
L["Totem Bar (Shaman)"] = "圖騰條（薩滿）"

-- Extra Bar (issue #330)
L["Extra Bar"] = "額外欄"
L["A 12-button action bar independent of every class's bonus bar (stance/stealth/vehicle)."] = "一個12格的動作列，獨立於任何職業的特殊列（形態/潛行/載具）。"
L["Fade the extra bar until you hover over it."] = "在滑鼠懸停之前淡出額外欄。"
L["Fade the extra bar until you enter combat."] = "在進入戰鬥之前淡出額外欄。"
L["Shamans only - Totem multicast bar. Position is controlled via Editor Mode."] = "僅限薩滿 - 圖騰多目標施法條。位置透過編輯模式控制。"
L["TIP: Use Editor Mode to position the totem bar (type /dragonui edit)."] = "提示：使用編輯模式來定點陣圖騰條（輸入 /dragonui edit）。"

-- ============================================================================
-- 施法條標籤頁
-- ============================================================================

L["Player Castbar"] = "玩家施法條"
L["Target Castbar"] = "目標施法條"
L["Focus Castbar"] = "焦點施法條"

-- 子標籤頁
L["Player"] = "玩家"
L["Focus"] = "焦點"

-- 通用選項
L["Width"] = "寬度"
L["Width of the cast bar"] = "施法條的寬度"
L["Height"] = "高度"
L["Height of the cast bar"] = "施法條的高度"
L["Scale"] = "縮放比例"
L["Size scale of the cast bar"] = "施法條的縮放比例"
L["Hide Castbar"] = "隱藏施法條"
L["Hide Castbar Desc"] = "完全隱藏此施法條：包括 DragonUI 施法條和暴雪預設施法條。"
L["Show Icon"] = "顯示圖示"
L["Show the spell icon next to the cast bar"] = "在施法條旁邊顯示法術圖示"
L["Show Spell Icon"] = "顯示法術圖示"
L["Show the spell icon next to the target castbar"] = "在目標施法條旁邊顯示法術圖示"
L["Icon Size"] = "圖示大小"
L["Size of the spell icon"] = "法術圖示的大小"
L["Text Mode"] = "文字模式"
L["Choose how to display spell text: Simple (centered spell name only) or Detailed (spell name + time)"] = "選擇法術文字的顯示方式：簡單（僅居中顯示法術名稱）或詳細（法術名稱 + 時間）"
L["Simple (Centered Name Only)"] = "簡單（僅居中顯示名稱）"
L["Simple (Name Only)"] = "簡單（僅顯示名稱）"
L["Simple"] = "簡單"
L["Detailed (Name + Time)"] = "詳細（名稱 + 時間）"
L["Detailed"] = "詳細"
L["Time Precision"] = "時間精度"
L["Decimal places for remaining time."] = "剩餘時間的小數點位數。"
L["Max Time Precision"] = "最大時間精度"
L["Decimal places for total time."] = "總時間的小數點位數。"
L["Hold Time (Success)"] = "保持時間（成功）"
L["How long the bar stays visible after a successful cast."] = "成功施法後施法條保持可見的時間。"
L["How long the bar stays after a successful cast."] = "施法成功後施法條保持的時間。"
L["How long to show the castbar after successful completion"] = "施法成功完成後施法條顯示的時間"
L["Hold Time (Interrupt)"] = "保持時間（打斷）"
L["How long the bar stays visible after being interrupted."] = "施法被打斷後施法條保持可見的時間。"
L["How long the bar stays after being interrupted."] = "施法被打斷後施法條保持的時間。"
L["How long to show the castbar after interruption/failure"] = "施法被打斷/失敗後施法條顯示的時間"
L["Auto-Adjust for Auras"] = "根據光環自動調整位置"
L["Automatically adjust position based on target auras (CRITICAL FEATURE)"] = "根據目標身上的光環自動調整位置（關鍵功能）"
L["Shift castbar when buff/debuff rows are showing."] = "當增益/減益效果行顯示時，移動施法條位置。"
L["Automatically adjust position based on focus auras"] = "根據焦點目標身上的光環自動調整位置"
L["Reset Position"] = "重置位置"
L["Resets the X and Y position to default."] = "將X和Y位置重置為預設值。"
L["Reset target castbar position to default"] = "將目標施法條位置重置為預設值"
L["Reset focus castbar position to default"] = "將焦點施法條位置重置為預設值"
L["Player castbar position reset."] = "玩家施法條位置已重置。"
L["Target castbar position reset."] = "目標施法條位置已重置。"
L["Focus castbar position reset."] = "焦點施法條位置已重置。"
L["Castbar detached - positioned freely via Editor Mode"] = "施法條已分離 - 可在編輯模式中自由定位"
L["Castbar attached - follows Target frame"] = "施法條已附著 - 跟隨目標框架"
L["Castbar attached - follows Focus frame"] = "施法條已附著 - 跟隨焦點框架"
L["Re-attach Castbar to Target"] = "將施法條重新附著到目標"
L["Re-attach Castbar to Focus"] = "將施法條重新附著到焦點"

-- 目標/焦點施法條的寬度/高度描述
L["Width of the target castbar"] = "目標施法條的寬度"
L["Height of the target castbar"] = "目標施法條的高度"
L["Scale of the target castbar"] = "目標施法條的縮放比例"
L["Width of the focus castbar"] = "焦點施法條的寬度"
L["Height of the focus castbar"] = "焦點施法條的高度"
L["Scale of the focus castbar"] = "焦點施法條的縮放比例"
L["Show the spell icon next to the focus castbar"] = "在焦點施法條旁邊顯示法術圖示"
L["Time to show the castbar after successful cast completion"] = "施法成功完成後施法條顯示的時間"
L["Time to show the castbar after cast interruption"] = "施法被打斷後施法條顯示的時間"

-- Latency indicator (player only)
L["Latency Indicator"] = "延遲指示器"
L["Enable Latency Indicator"] = "啟用延遲指示器"
L["Show a safe-zone overlay based on real cast latency."] = "基於實際施法延遲顯示安全區域。"
L["Latency Color"] = "延遲顏色"
L["Latency Alpha"] = "延遲透明度"
L["Opacity of the latency indicator."] = "延遲指示器的透明度。"

-- ============================================================================
-- 增強功能標籤頁
-- ============================================================================

L["Enhancements"] = "增強功能"
L["Visual enhancements that add Dragonflight-style polish to the UI. These are optional — disable any you don't want."] = "為介面新增巨龍時代風格潤色的視覺增強功能。這些是可選的——可禁用任何你不想要的功能。"

-- （暗色模式、距離指示器、物品品質、滑鼠提示已在模組部分定義）

-- ============================================================================
-- 微型選單標籤頁
-- ============================================================================

L["Gray Scale Icons"] = "灰度圖示"
L["Grayscale Icons"] = "灰度圖示"
L["Use grayscale icons instead of colored icons for the micro menu"] = "為微型選單使用灰度圖示而非彩色圖示"
L["Use grayscale icons instead of colored icons."] = "使用灰度圖示而非彩色圖示。"
L["Grayscale Icons Settings"] = "灰度圖示設定"
L["Normal Icons Settings"] = "普通圖示設定"
L["Menu Scale"] = "選單縮放"
L["Icon Spacing"] = "圖示間距"
L["Padding between micromenu buttons."] = "微型選單按鈕之間的間距。"
L["Number of micromenu button columns. Set to 1 for a vertical stack."] = "微型選單欄數。設為 1 則為直向排列。"
L["Invert Button Order"] = "反轉按鈕順序"
L["Reverse the order of micromenu buttons in the grid."] = "反轉網格中微型選單按鈕的順序。"
L["Hide on Vehicle"] = "在載具上時隱藏"
L["Hide micromenu and bags if you sit on vehicle"] = "如果你在載具上，則隱藏微型選單和揹包"
L["Hide micromenu and bags while in a vehicle."] = "在載具上時隱藏微型選單和揹包。"
L["Show Latency Indicator"] = "顯示延遲指示器"
L["Show a colored bar below the Help button indicating connection quality (green/yellow/red). Requires UI reload."] = "在幫助按鈕下方顯示指示連線質量的彩色條（綠色/黃色/紅色）。需要重新載入介面。"

-- 揹包
L["Bags"] = "揹包"
L["Configure the position and scale of the bag bar independently from the micro menu."] = "獨立於微型選單配置揹包欄的位置和縮放。"
L["Bag Bar Scale"] = "揹包欄縮放"

-- 經驗值和聲望條
L["XP Bar"] = "經驗值條"
L["XP & Rep Bars (Legacy Offsets)"] = "經驗值和聲望條（舊版偏移量）"
L["Main XP & Rep bar options have moved to the XP & Rep Bars tab."] = "主要的經驗值和聲望條選項已移至“經驗值和聲望條”標籤頁。"
L["These offset options are for advanced positioning adjustments."] = "這些偏移量選項用於高階位置微調。"
L["Both Bars Offset"] = "雙條偏移量"
L["Y offset when XP & reputation bar are shown"] = "當經驗值和聲望條都顯示時的Y軸偏移量"
L["Single Bar Offset"] = "單條偏移量"
L["Y offset when XP or reputation bar is shown"] = "當經驗值或聲望條顯示時的Y軸偏移量"
L["No Bar Offset"] = "無條偏移量"
L["Y offset when no XP or reputation bar is shown"] = "當經驗值或聲望條均不顯示時的Y軸偏移量"
L["Rep Bar Above XP Offset"] = "經驗條上方的聲望條偏移量"
L["Y offset for reputation bar when XP bar is shown"] = "當經驗條顯示時聲望條的Y軸偏移量"
L["Rep Bar Offset"] = "聲望條偏移量"
L["Y offset when XP bar is not shown"] = "當經驗條不顯示時的Y軸偏移量"

-- ============================================================================
-- 小地圖示籤頁
-- ============================================================================

L["Basic Settings"] = "基本設定"
L["Border Alpha"] = "邊框透明度"
L["Top border alpha (0 to hide)."] = "頂部邊框的透明度（0為完全隱藏）。"
L["Square Minimap"] = "方形小地圖"
L["Use a square minimap with a square border instead of the round DragonUI border."] = "使用方形小地圖和方形邊框，而非圓形的DragonUI邊框。"
L["Addon Button Skin"] = "外掛按鈕皮膚"
L["Apply DragonUI border styling to addon icons (e.g., bag addons)"] = "為外掛圖示（例如揹包外掛）應用DragonUI邊框樣式"
L["Apply DragonUI border styling to addon icons."] = "為外掛圖示應用DragonUI邊框樣式。"
L["Addon Button Fade"] = "外掛按鈕淡出"
L["Addon icons fade out when not hovered."] = "外掛圖示在未懸停時淡出。"
L["Player Arrow Size"] = "玩家箭頭大小"
L["Size of the player arrow on the minimap"] = "小地圖上玩家箭頭的大小"
L["New Blip Style"] = "新圖示樣式"
L["Use new DragonUI object icons on the minimap. When disabled, uses classic Blizzard icons."] = "在小地圖上使用新的DragonUI物件圖示。禁用時，使用經典的暴雪圖示。"
L["Use newer-style minimap blip icons."] = "使用新樣式的小地圖圖示。"
L["Animated Border"] = "動畫邊框"
L["Minimap Decorations"] = "小地圖裝飾"
L["Adds decorative animated texture layers around the DragonUI minimap."] = "在 DragonUI 小地圖周圍加入裝飾性動畫材質層。"
L["Enable Animated Border"] = "啟用動畫邊框"
L["Enable Minimap Decorations"] = "啟用小地圖裝飾"
L["Animated Effects"] = "動畫效果"
L["Rotate preset layers when the selected preset includes animation."] = "當選取的預設包含動畫時，旋轉預設圖層。"
L["Hide DragonUI Border"] = "隱藏 DragonUI 邊框"

-- 時間與日曆
L["Time & Calendar"] = "時間與日曆"
L["Show Clock"] = "顯示時鐘"
L["Show/hide the minimap clock"] = "顯示/隱藏小地圖時鐘"
L["Show Calendar"] = "顯示日曆"
L["Show/hide the calendar frame"] = "顯示/隱藏日曆框架"
L["Clock Font Size"] = "時鐘字型大小"
L["Font size for the clock numbers on the minimap"] = "小地圖上時鐘數字的字型大小"

-- 顯示設定
L["Display Settings"] = "顯示設定"
L["Tracking Icons"] = "追蹤圖示"
L["Show current tracking icons (old style)."] = "顯示當前的追蹤圖示（舊樣式）。"
L["Zoom Buttons"] = "縮放按鈕"
L["Show zoom buttons (+/-)."] = "顯示縮放按鈕 (+/-)。"
L["Zone Text Size"] = "區域文字大小"
L["Zone Text Font Size"] = "區域文字字型大小"
L["Zone text font size on top border"] = "頂部邊框上區域文字的字型大小"
L["Font size of the zone text above the minimap."] = "小地圖上方區域文字的字型大小設定。"

-- 位置
L["Position"] = "位置"
L["Reset minimap to default position (top-right corner)"] = "將小地圖重置到預設位置（右上角）"
L["Reset Minimap Position"] = "重置小地圖位置"
L["Minimap position reset to default"] = "小地圖位置已重置為預設值"
L["Minimap position reset."] = "小地圖位置已重置。"

-- ============================================================================
-- 任務追蹤器標籤頁
-- ============================================================================

L["Configures the quest objective tracker position and behavior."] = "配置任務目標追蹤器的位置和行為。"
L["Position and display settings for the objective tracker."] = "任務追蹤器的位置和顯示設定。"
L["Show Header Background"] = "顯示標題背景"
L["Show/hide the decorative header background texture."] = "顯示/隱藏裝飾性的標題背景紋理。"
L["Anchor Point"] = "錨點"
L["Screen anchor point for the quest tracker."] = "任務追蹤器的螢幕錨點。"
L["Top Right"] = "右上"
L["Top Left"] = "左上"
L["Bottom Right"] = "右下"
L["Bottom Left"] = "左下"
L["Center"] = "中央"
L["Horizontal position offset"] = "水平位置偏移量"
L["Vertical position offset"] = "垂直位置偏移量"
L["Reset quest tracker to default position"] = "將任務追蹤器重置為預設位置"
L["Font size for quest tracker text"] = "任務追蹤器文字的字型大小設定"

-- ============================================================================
-- 單位框架標籤頁
-- ============================================================================

-- 子標籤頁
L["Pet"] = "寵物"
L["ToT / ToF"] = "目標的目標 / 焦點的目標"
L["Party"] = "小隊"

-- 通用選項
L["Global Scale"] = "全域性縮放"
L["Global scale for all unit frames"] = "所有單位框架的全域性縮放"
L["Scale of the player frame"] = "玩家框架的縮放"
L["Scale of the target frame"] = "目標框架的縮放"
L["Scale of the focus frame"] = "焦點框架的縮放"
L["Scale of the pet frame"] = "寵物框架的縮放"
L["Scale of the target of target frame"] = "目標的目標框架的縮放"
L["Scale of the focus of target frame"] = "焦點的目標框架的縮放"
L["Scale of party frames"] = "小隊框架的縮放"
L["Class Color"] = "職業顏色"
L["Class Color Health"] = "生命條職業顏色"
L["Use class color for health bar"] = "生命條使用職業顏色"
L["Use class color for health bars in party frames"] = "小隊框架的生命條使用職業顏色"
L["Class Portrait"] = "職業頭像"
L["Show class icon instead of 3D portrait"] = "顯示職業圖示而非3D頭像"
L["Show class icon instead of 3D portrait (only for players)"] = "顯示職業圖示而非3D頭像（僅限玩家）"
L["Class icon instead of 3D model for players."] = "玩家頭像使用職業圖示而非3D模型。"
L["Alternative Class Icons"] = "替代職業圖示"
L["Use DragonUI alternative class icons instead of Blizzard's class icon atlas."] = "使用 DragonUI 的替代職業圖示，而不是 Blizzard 的職業圖示圖集。"
L["Large Numbers"] = "大數字縮寫"
L["Format Large Numbers"] = "大數字縮寫格式"
L["Format large numbers (1k, 1m)"] = "大數字縮寫格式 (1k, 1m)"
L["Text Format"] = "文字格式"
L["How to display health and mana values"] = "生命值和法力值的顯示方式"
L["Choose how to display health and mana text"] = "選擇生命值和法力值文字的顯示方式"

-- 文字格式值
L["Current Value Only"] = "僅當前值"
L["Current Value"] = "當前值"
L["Percentage Only"] = "僅百分比"
L["Percentage"] = "百分比"
L["Both (Numbers + Percentage)"] = "兩者（數值 + 百分比）"
L["Numbers + %"] = "數值 + %"
L["Current/Max Values"] = "當前/最大值"
L["Current / Max"] = "當前 / 最大"

-- 小隊文字格式值
L["Current Value Only (2345)"] = "僅當前值 (2345)"
L["Formatted Current (2.3k)"] = "縮寫的當前值 (2.3k)"
L["Percentage Only (75%)"] = "僅百分比 (75%)"
L["Percentage + Current (75% | 2.3k)"] = "百分比 + 當前值 (75% | 2.3k)"

-- 生命值/法力值文字
L["Always Show Health Text"] = "始終顯示生命值文字"
L["Show health text always (true) or only on hover (false)"] = "始終顯示生命值文字（是）或僅在懸停時顯示（否）"
L["Always show health text on party frames (instead of only on hover)"] = "在小隊框架上始終顯示生命值文字（而不僅是懸停時）"
L["Always display health text (otherwise only on mouseover)"] = "始終顯示生命值文字（預設：僅滑鼠懸停時顯示）"
L["Always Show Mana Text"] = "始終顯示法力值文字"
L["Show mana/power text always (true) or only on hover (false)"] = "始終顯示法力值/能量文字（是）或僅在懸停時顯示（否）"
L["Always show mana text on party frames (instead of only on hover)"] = "在小隊框架上始終顯示法力值文字（而不僅是懸停時）"
L["Always display mana/energy/rage text (otherwise only on mouseover)"] = "始終顯示法力值/能量/怒氣文字（預設：僅滑鼠懸停時顯示）"

-- 玩家框架特定選項
L["Player Frame"] = "玩家框架"
L["Dragon Decoration"] = "龍形裝飾"
L["Add decorative dragon to your player frame for a premium look"] = "為你的玩家框架新增裝飾性的龍，以獲得高階外觀"
L["None"] = "無"
L["Elite Dragon (Golden)"] = "精英龍（金色）"
L["Elite (Golden)"] = "精英（金色）"
L["RareElite Dragon (Winged)"] = "稀有精英龍（有翼）"
L["RareElite (Winged)"] = "稀有精英（有翼）"
L["Glow Effects"] = "發光效果"
L["Show Rest Glow"] = "顯示休息時發光效果"
L["Show a golden glow around the player frame when resting (in an inn or city). Works with all frame modes: normal, elite, fat health bar, and vehicle."] = "休息時（在旅店或城市中）在玩家框架周圍顯示金色發光效果。適用於所有框架模式：普通、精英、寬體生命條和載具。"
L["Golden glow around the player frame when resting (inn or city). Works with all frame modes."] = "休息時（在旅店或城市中）在玩家框架周圍顯示金色發光效果。適用於所有框架模式。"
L["Combat Flash"] = "戰鬥閃光"
L["Show Combat Flash"] = "顯示戰鬥閃光"
L["Pulsing glow effect when entering combat. Works with all frame modes."] = "進入戰鬥時的脈衝發光效果。適用於所有框架模式。"
L["Combat Flash Opacity"] = "戰鬥閃光不透明度"
L["Maximum opacity of the combat flash pulse effect."] = "戰鬥閃光脈衝效果的最大不透明度。"
L["Always Show Alternate Mana Text"] = "始終顯示備用法力文字"
L["Show mana text always visible (default: hover only)"] = "始終顯示法力值文字（預設：僅懸停時顯示）"
L["Alternate Mana (Druid)"] = "備用法力（德魯伊）"
L["Always Show"] = "始終顯示"
L["Druid mana text visible at all times, not just on hover."] = "德魯伊的法力值文字始終可見，而不僅是懸停時。"
L["Alternate Mana Text Format"] = "備用法力文字格式"
L["Choose text format for alternate mana display"] = "選擇備用法力顯示的文字格式"
L["Percentage + Current/Max"] = "百分比 + 當前/最大值"

-- 寬體生命條
L["Health Bar Style"] = "生命條樣式"
L["Fat Health Bar"] = "寬體生命條"
L["Enable"] = "啟用"
L["Full-width health bar that fills the entire frame area. Uses modified border texture that removes the inner divider line. Compatible with Dragon Decoration (requires fat variant textures). Note: Automatically disabled during vehicle UI."] = "填充整個框架區域的寬體生命條。使用移除了內部分隔線的修改邊框紋理。與龍形裝飾相容（需要寬體變體紋理）。注意：在載具介面期間會自動禁用。"
L["Full-width health bar. Auto-disabled in vehicles."] = "寬體生命條。乘坐載具時自動禁用。"
L["Hide Mana Bar (Fat Mode)"] = "隱藏法力條（寬體模式）"
L["Hide Mana Bar"] = "隱藏法力條"
L["Completely hide the mana bar when Fat Health Bar is active."] = "當寬體生命條啟用時，完全隱藏法力條。"
L["Mana Bar Width (Fat Mode)"] = "法力條寬度（寬體模式）"
L["Mana Bar Width"] = "法力條寬度"
L["Width of the mana bar when Fat Health Bar is active. Movable via Editor Mode."] = "寬體生命條啟用時，法力條的寬度。可透過編輯模式移動。"
L["Mana Bar Height (Fat Mode)"] = "法力條高度（寬體模式）"
L["Mana Bar Height"] = "法力條高度"
L["Height of the mana bar when Fat Health Bar is active."] = "寬體生命條啟用時，法力條的高度。"
L["Mana Bar Texture"] = "法力條紋理"
L["Choose the texture style for the power/mana bar. Only applies in Fat Health Bar mode."] = "選擇能量/法力條的紋理樣式。僅適用於寬體生命條模式。"
L["DragonUI (Default)"] = "DragonUI（預設）"
L["Blizzard Classic"] = "暴雪經典"
L["Flat Solid"] = "純色扁平"
L["Smooth"] = "平滑"
L["Aluminium"] = "鋁製"
L["LiteStep"] = "輕步"

-- 能量條顏色
L["Power Bar Colors"] = "能量條顏色"
L["Mana"] = "法力"
L["Rage"] = "怒氣"
L["Energy"] = "能量"
L["Runic Power"] = "符文能量"
L["Happiness"] = "快樂值"
L["Runes"] = "符文"
L["Reset Colors to Default"] = "重置顏色為預設值"

-- 目標框架
L["Target Frame"] = "目標框架"
L["Threat Glow"] = "仇恨發光效果"
L["Show threat glow effect"] = "顯示仇恨發光效果"
L["Show Name Background"] = "顯示名稱背景"
L["Show the colored name background behind the target name."] = "在目標名稱後面顯示有顏色的名稱背景。"

-- 焦點框架
L["Focus Frame"] = "焦點框架"
L["Show the colored name background behind the focus name."] = "在焦點名稱後面顯示有顏色的名稱背景。"
L["Show Buff/Debuff on Focus"] = "在焦點上顯示增益/減益"
L["Uses the native large focus frame mode to show buffs and debuffs on the focus frame."] = "使用原生的大型焦點框架模式，在焦點框架上顯示增益和減益。"
L["Override Position"] = "覆蓋預設位置"
L["Override default positioning"] = "覆蓋預設位置設定"
L["Move the pet frame independently from the player frame."] = "將寵物框架與玩家框架分開移動。"

-- 寵物框架
L["Pet Frame"] = "寵物框架"
L["Allows the pet frame to be moved freely. When unchecked, it will be positioned relative to the player frame."] = "允許寵物框架自由移動。不勾選時，它將相對於玩家框架定位。"
L["Horizontal position (only active if Override is checked)"] = "水平位置（僅在“覆蓋預設位置”勾選時生效）"
L["Vertical position (only active if Override is checked)"] = "垂直位置（僅在“覆蓋預設位置”勾選時生效）"

-- 目標的目標
L["Target of Target"] = "目標的目標"
-- Player Stats
L["Player Stats"] = "玩家屬性"
L["Add item level, PvE/PvP power and prestige for player units."] = "為玩家單位顯示物品等級、PvE/PvP 強度與威望。"
L["Follows the Target frame by default. Move it in Editor Mode (/dragonui edit) to detach and position freely."] = "預設跟隨目標框架。在編輯模式（/dragonui edit）中移動它可使其分離並自由定位。"
L["Detached — positioned freely via Editor Mode"] = "已分離 — 透過編輯模式自由定位"
L["Attached — follows Target frame"] = "已附加 — 跟隨目標框架"
L["Re-attach to Target"] = "重新附加到目標框架"

-- 焦點的目標
L["Target of Focus"] = "焦點的目標"
L["Follows the Focus frame by default. Move it in Editor Mode (/dragonui edit) to detach and position freely."] = "預設跟隨焦點框架。在編輯模式（/dragonui edit）中移動它可使其分離並自由定位。"
L["Attached — follows Focus frame"] = "已附加 — 跟隨焦點框架"
L["Re-attach to Focus"] = "重新附加到焦點框架"

-- 小隊框架
L["Party Frames"] = "小隊框架"
L["Party Frames Configuration"] = "小隊框架配置"
L["Custom styling for party member frames with automatic health/mana text display and class colors."] = "具有自動生命值/法力值文字顯示和職業顏色的自定義小隊成員框架樣式。"

-- 首領框架
L["Boss Frames"] = "首領框架"
L["Enabled"] = "已啟用"

L["Orientation"] = "排列方向"
L["Vertical"] = "垂直"
L["Horizontal"] = "水平"
L["Party frame orientation"] = "小隊框架排列方向"
L["Vertical Padding"] = "垂直間距"
L["Space between party frames in vertical mode."] = "垂直排列模式下，小隊框架之間的間距。"
L["Horizontal Padding"] = "水平間距"
L["Space between party frames in horizontal mode."] = "水平排列模式下，小隊框架之間的間距。"

-- ============================================================================
-- 經驗值和聲望條標籤頁
-- ============================================================================

L["Bar Style"] = "條樣式"
L["XP / Rep Bar Style"] = "經驗值 / 聲望條樣式"
L["DragonflightUI: fully custom bars with rested XP background.\nRetailUI: atlas-based reskin of Blizzard bars.\n\nChanging style requires a UI reload."] = "DragonflightUI：帶有休息經驗值背景的完全自定義條。\nRetailUI：基於圖集的暴雪條重製版。\n\n更改樣式需要重新載入介面。"
L["DragonflightUI"] = "巨龍時代UI"
L["RetailUI"] = "正式服UI"
L["XP bar style changed to "] = "經驗條樣式更改為："
L["A UI reload is required to apply this change."] = "需要重新載入介面以應用此更改。"

-- 大小和縮放
L["Size & Scale"] = "大小和縮放"
L["Height of the XP and Reputation bars (in pixels)."] = "經驗值和聲望條的高度（以畫素為單位）。"
L["Width of the XP and Reputation bars (in pixels)."] = "經驗值和聲望條的寬度（以畫素為單位）。"
L["Experience Bar Scale"] = "經驗條縮放"
L["Scale of the experience bar."] = "經驗條的縮放比例。"
L["Reputation Bar Scale"] = "聲望條縮放"
L["Scale of the reputation bar."] = "聲望條的縮放比例。"

-- 休息經驗值
L["Rested XP"] = "休息經驗值"
L["Show Rested XP Background"] = "顯示休息經驗值背景"
L["Display a translucent bar showing the total available rested XP range.\n(DragonflightUI style only)"] = "顯示一個半透明條，指示可用的總休息經驗值範圍。\n（僅限巨龍時代UI樣式）"
L["Show Exhaustion Tick"] = "顯示休息狀態分界線"
L["Show the exhaustion tick indicator on the XP bar, marking where rested XP ends."] = "在經驗條上顯示休息狀態分界線指示器，標記休息經驗值結束的位置。"

-- 文字顯示
L["Text Display"] = "文字顯示"
L["Always Show Text"] = "始終顯示文字"
L["Always display XP/Rep text instead of only on hover."] = "始終顯示經驗值/聲望文字，而不僅是滑鼠懸停時。"
L["Show XP Percentage"] = "顯示經驗值百分比"
L["Display XP percentage alongside the value text."] = "在數值文字旁邊顯示經驗值百分比。"

-- ============================================================================
-- 配置檔案標籤頁
-- ============================================================================

L["Database not available."] = "資料庫不可用。"
L["Save and switch between different configurations per character."] = "為每個角色儲存和切換不同的配置。"
L["Current Profile"] = "當前配置檔案"
L["Active: "] = "啟用的配置檔案："
L["Switch or Create Profile"] = "切換或建立配置檔案"
L["Select Profile"] = "選擇配置檔案"
L["New Profile Name"] = "新配置檔名"
L["Default Profile"] = "預設配置檔案"
L["New characters will use this profile."] = "新角色將使用此配置檔案。"
L["New characters will get their own profile."] = "新角色將使用各自的配置檔案。"
L["New characters will start on profile: "] = "新角色將使用配置檔案："
L["Copy From"] = "從何處複製"
L["Copies all settings from the selected profile into your current one."] = "將所選配置檔案的所有設定複製到當前配置檔案中。"
L["Copied profile: "] = "已複製的配置檔案："
L["Delete Profile"] = "刪除配置檔案"
L["Warning: Deleting a profile is permanent and cannot be undone."] = "警告：刪除配置檔案是永久性的，無法撤銷。"
L["Delete"] = "刪除"
L["Deleted profile: "] = "已刪除的配置檔案："
L["Are you sure you want to delete the profile '%s'? This cannot be undone."] = "你確定要刪除配置檔案 '%s' 嗎？此操作無法撤銷。"
L["Profile Manager"] = true
L["Manage your current profile: reset to defaults, export/import as text, or share in-game."] = true
L["Restores the current profile to defaults."] = "將當前配置檔案恢復到其預設設定。"
L["Export your current profile as a text string."] = "將目前設定檔匯出為文字字串。"

L["Reset Profile"] = "重置配置檔案"
L["All changes will be lost and the UI will be reloaded.\nAre you sure you want to reset your profile?"] = "所有更改都將丟失，並且介面將重新載入。\n你確定要重置你的配置檔案嗎？"
L["Profile reset to defaults."] = "配置檔案已重置為預設值。"

-- 單位框架層模組
L["Unit Frame Layers"] = "單位框架層"
L["Enable Unit Frame Layers"] = "啟用單位框架層"
L["Heal prediction, absorb shields, and animated health loss on unit frames"] = "單位框架上的治療預估、吸收護盾和動態生命值損失"
L["Heal prediction bars, absorb shields, and animated health loss overlays on unit frames."] = "單位框架上的治療預估條、吸收護盾和動態生命值損失覆蓋層。"
L["Show heal prediction, absorb shields, and animated health loss on all unit frames."] = "在所有單位框架上顯示治療預估、吸收護盾和動態生命值損失。"
L["Animated Health Loss"] = "動態生命值損失"
L["Show animated red health loss bar on player frame when taking damage."] = "受到傷害時，在玩家框架上顯示紅色的動態生命值損失條。"
L["Missing Health Text"] = "缺失生命值文字"
L["Show the health deficit (missing health) as red text on health bars. Useful for healers."] = "在生命值條上以紅色文字顯示生命值缺口（缺失生命值）。對治療者很有用。"
L["Builder/Spender Feedback"] = "資源獲取/消耗反饋"
L["Show mana gain/loss glow feedback on player mana bar (experimental)."] = "在玩家法力條上顯示法力獲取/消耗的發光反饋（實驗性）。"

-- Preset (used by minimap tab)
L["Preset"] = "預設"
L["Save"] = "儲存"
L["Load"] = "載入"
L["Load preset '%s'? This will overwrite your current layout settings."] = "載入預設 '%s'？這將覆蓋您當前的佈局設定。"
L["Load position preset '%s'? This will overwrite your current element positions."] = "載入預設 '%s'？這將覆蓋目前元素位置。"
L["Load Preset"] = "載入預設"
L["Delete preset '%s'? This cannot be undone."] = "刪除預設 '%s'？此操作無法撤銷。"
L["Delete position preset '%s'? This cannot be undone."] = "刪除預設 '%s'？此操作無法復原。"
L["Delete Preset"] = "刪除預設"
L["Duplicate Preset"] = "複製預設"
L["Preset saved: "] = "預設已儲存: "
L["Preset loaded: "] = "預設已載入: "
L["Preset deleted: "] = "預設已刪除: "
L["Preset duplicated: "] = "預設已複製: "
L["Also delete all saved layout presets?"] = "是否同時刪除所有已儲存的佈局預設？"
L["Also delete all saved position presets?"] = "是否同時刪除所有已儲存的位置預設？"
L["Presets kept."] = "預設已保留。"
L["Position presets kept."] = "位置預設已保留。"

-- PRESET IMPORT / EXPORT
L["Export Preset"] = "匯出預設"
L["Import Preset"] = "匯入預設"
L["Import a preset from a text string shared by another player."] = "從其他玩家分享的文字中匯入預設。"
L["Position Presets"] = "位置預設"
L["Position Preset"] = "位置預設"
L["Export Position Preset"] = "匯出位置預設"
L["Import Position Preset"] = "匯入位置預設"
L["No position presets saved yet. Save one in Edit Mode (/dragonui edit) first."] = "尚未儲存位置預設。請先在編輯模式 (/dragonui edit) 中儲存一個。"
L["Position presets not available."] = "位置預設不可用。"
L["Failed to export position preset."] = "匯出位置預設失敗。"
L["Invalid position preset string."] = "無效的位置預設字串。"
L["Not a valid DragonUI position preset string."] = "不是有效的 DragonUI 位置預設字串。"
L["Import"] = "匯入"
L["Select All"] = "全選"
L["Close"] = "關閉"

-- Fade / hover settings (actionbars, micromenu, bags)
L["Hover/Combat Logic"] = "懸停/戰鬥邏輯"
L["When both hover and combat are enabled, choose whether both are required (AND) or either condition is enough (OR)."] = "當懸停與戰鬥同時啟用時，選擇需同時滿足（AND）或符合其一即可（OR）。"
L["AND (both required)"] = "AND（需同時滿足）"
L["OR (either condition)"] = "OR（符合其一）"
L["Bag Bar"] = "揹包欄"
L["Hover Fade"] = "懸停淡隱"
L["Visible Alpha"] = "顯示透明度"
L["Opacity when a bar is considered visible by hover/combat rules."] = "當動作條依懸停/戰鬥規則判定為可見時的透明度。"
L["Hidden Alpha"] = "隱藏透明度"
L["Opacity when a bar is hidden by hover/combat rules. Set above 0 to keep bars faintly visible."] = "當動作條依懸停/戰鬥規則判定為隱藏時的透明度。設為大於0可保持微弱可見。"
L["Fade In Duration"] = "淡入時長"
L["Seconds used to fade bars in when they become visible."] = "動作條變為可見時使用的淡入秒數。"
L["Fade Out Duration"] = "淡出時長"
L["Seconds used to fade bars out when they become hidden."] = "動作條變為隱藏時使用的淡出秒數。"
L["Fade Out Delay"] = "淡出延遲"
L["Delay before hover-out starts fading, useful to avoid flicker between buttons."] = "滑鼠移出後開始淡出的延遲時間，可減少按鈕間閃爍。"

-- PROFILE IMPORT/EXPORT
L["Import Profile"] = "匯入設定檔"
L["Import"] = "匯入"
L["Export Profile"] = "匯出設定檔"
L["Exporting..."] = true
L["Sending..."] = true
L["Failed to export profile."] = "匯出設定檔失敗。"
L["Import a profile from a text string shared by another user."] = "從其他使用者分享的文字字串匯入設定檔。"
L["Invalid profile string."] = "無效的設定檔字串。"
L["Not a valid DragonUI profile string."] = "不是有效的 DragonUI 設定檔字串。"
L["Profile imported: "] = "設定檔已匯入: "
L["Enter a name for the imported profile:"] = "為匯入的設定檔輸入名稱："
L["Imported Profile"] = "匯入的設定檔"
L["Enter profile name"] = "輸入設定檔名稱"
L["This Fork is maintained by PentSec for AscensionWow, based on the original work by Neticsoul."] = "此分支由 PentSec 為 AscensionWow 維護，基於 Neticsoul 的原始工作。"

-- Low HP Alert
L["Low HP Alert"] = "低血量警報"
L["Plays a sound and flashes the screen edges when your HP drops below the configured threshold."] = "當你的生命值低於設定閾值時，播放聲音並閃爍螢幕邊緣。"
L["Enable Low HP Alert"] = "啟用低血量警報"
L["Warning Sound (every 3 sec)"] = "警告音（每3秒）"
L["Plays a warning sound every 3 seconds while HP is below threshold."] = "當生命值低於閾值時，每3秒播放一次警告音。"
L["Screen Edge Flash"] = "螢幕邊緣閃爍"
L["Flashes red at the edges of the screen while HP is below threshold."] = "當生命值低於閾值時，螢幕邊緣閃爍紅色。"
L["HP Warning Threshold"] = "血量警告閾值"
L["Percentage of HP at which the warning triggers."] = "觸發警告的生命值百分比。"
L["Test Warning (3 sec)"] = "測試警告（3秒）"
L["Triggers a 3-second preview of the warning state to help you adjust the threshold."] = "觸發3秒的警告狀態預覽，幫助調整閾值。"
L["Flash Color"] = "閃爍顏色"
L["Color of the screen edge flash effect."] = "螢幕邊緣閃爍效果的顏色。"
L["Use Class Color"] = "使用職業顏色"
L["Use your class color (e.g. orange for Druid, yellow for Rogue) instead of the custom color."] = "使用你的職業顏色（例如德魯伊橙色、盜賊黃色）替代自訂顏色。"
L["Flash Opacity"] = "閃爍不透明度"
L["Maximum opacity of the flash effect."] = "閃爍效果的最大不透明度。"
L["Flash Extent"] = "閃爍範圍"
L["How far the flash extends from the screen edges toward the center, in pixels."] = "閃爍從螢幕邊緣向中心延伸的距離（像素）。"

-- BNet Toast Tab
L["BNet Toast"] = "BNet 提醒"
L["Friend online/offline notifications with Battle.net toasts and chat messages"] = "好友上線/離線的戰網提醒和聊天通知"
L["Position & Scale"] = "位置與縮放"
L["Scale of the BNet toast frame."] = "戰網提示框架的縮放比例。"
L["Horizontal position of the BNet toast from the screen center. Negative values move left, positive values move right."] = "戰網提示距螢幕中心的水平位置。負值向左移動，正值向右移動。"
L["Vertical offset of the BNet toast frame. Negative values move down, positive values move up."] = "戰網提示框架的垂直偏移。負值向下移動，正值向上移動。"
L["Close"] = "關閉"
L["Enter a name for the imported preset:"] = "為匯入的預設輸入名稱："
L["Imported Preset"] = "匯入的預設"
L["Preset imported: "] = "預設已匯入: "
L["Invalid preset string."] = "無效的預設字串。"
L["Not a valid DragonUI preset string."] = "不是有效的 DragonUI 預設字串。"
L["Failed to export preset."] = "匯出預設失敗。"

-- ============================================================================
-- NAMEPLATES
-- ============================================================================

L["Nameplates"] = "名牌"
L["Unit Nameplates"] = "單位名牌"
L["Friendly Units"] = "友方單位"
L["Enemy Units"] = "敵方單位"
L["Pets"] = "寵物"
L["Guardians"] = "守護者"
L["Totems"] = "圖騰"
L["Enable Nameplates Module"] = "啟用名牌模組"
L["Enable Nameplates"] = "啟用名牌"
L["Enable or disable the DragonUI nameplate module."] = "啟用或停用 DragonUI 名牌模組。"
L["Apply DragonUI nameplate styling."] = "將 DragonUI 樣式套用至名牌。"
L["DragonUI-style health bars on Blizzard nameplates (30300)."] = "暴雪名牌上的 DragonUI 風格生命條 (30300)。"
L["Offset X"] = "X 偏移"
L["Offset Y"] = "Y 偏移"
L["Show Level in Name"] = "在名字中顯示等級"
L["Side arrows on the targeted nameplate."] = "目標名牌上的側邊箭頭。"
L["Bar Size"] = "條大小"
L["Bar Width"] = "條寬度"
L["Width of the nameplate health bar."] = "名牌生命條的寬度。"
L["Bar Height"] = "條高度"
L["Height of the nameplate health bar."] = "名牌生命條的高度。"
L["Font Size"] = "字型大小"
L["Name and health percent font scale (1-10)."] = "名字和生命百分比字型縮放 (1-10)。"
L["Stack Offset X"] = "堆疊 X 偏移"
L["Horizontal offset for the nameplate stack."] = "名牌堆疊的水平偏移。"
L["Stack Offset Y"] = "堆疊 Y 偏移"
L["Vertical offset for the nameplate stack."] = "名牌堆疊的垂直偏移。"
L["Display"] = "顯示"
L["Show Health Percent"] = "顯示生命百分比"
L["Show Health Number"] = "顯示血量數值"
L["Shows HP as a number (e.g. 22k) and percent on the health bar."] = "在血量條上以數值（如22k）和百分比顯示當前血量。"
L["Health Number Font Size"] = "血量數值字體大小"
L["Health number font scale (1-10)."] = "血量數值字體大小（1-10）。"
L["Center Name Only"] = "僅居中顯示名稱"
L["Centers the unit name and hides the health percent."] = "置中顯示名稱並隱藏血量百分比。"
L["Hides level text and health percent, and centers the unit name on the nameplate."] = "隱藏等級和生命百分比，並在名牌上居中顯示單位名稱。"
L["Gray Tapped Units"] = "已被他人佔領時變灰"
L["Grays the health bar when a unit is tapped by another player or group."] = "當單位被其他玩家或隊伍佔領時，將生命條顯示為灰色。"
L["Friendly Player Color"] = "友方玩家顏色"
L["Friendly NPC Color"] = "友方 NPC 顏色"
L["Party Class Colors"] = "隊伍職業顏色"
L["Use class colors for party member nameplates instead of the friendly player color."] = "對隊伍成員名牌使用職業顏色而非友方玩家顏色。"
L["Friendly Class Colors"] = "友方職業顏色"
L["Class-color every friendly player, not just your group. Party and raid show automatically; others fill in when you target or mouse over them, or instantly with awesome_wotlk."] = "對所有友方玩家使用職業顏色，不僅限於隊伍。隊伍和團隊自動生效；其他玩家在選取或滑鼠懸停時生效，或使用 awesome_wotlk 時即時生效。"
L["Headline Mode"] = "僅名稱模式"
L["Enable Headline Mode"] = "啟用僅名稱模式"
L["Hide health, power and cast bars on friendly nameplates, showing only the name."] = "隱藏友方名牌上的生命、能量和施法條，僅顯示名稱。"
L["Name Text Color"] = "名稱文字顏色"
L["Friendly Players"] = "友方玩家"
L["Class colors, title, guild and AFK read from the unit: party/raid show automatically; others fill in when you target or mouse over them, or instantly with awesome_wotlk."] = "職業顏色、頭銜、公會和暫離皆讀取自單位：小隊/團隊自動顯示；其他玩家在選取或滑鼠停留時填入，使用 awesome_wotlk 則立即顯示。"
L["Party / Raid Members"] = "小隊/團隊成員"
L["Apply headline mode to your party and raid members."] = "對你的小隊和團隊成員套用僅名稱模式。"
L["All Friendly Players"] = "所有友方玩家"
L["Apply headline mode to all friendly players, not just your group."] = "對所有友方玩家套用僅名稱模式，而不僅是你的隊伍。"
L["Show Class Colors"] = "顯示職業顏色"
L["Show friendly player names in their class color."] = "以職業顏色顯示友方玩家的名稱。"
L["Show Player Title"] = "顯示玩家頭銜"
L["Show the player's title with their name (e.g. \"Arthas Jenkins\")."] = "在玩家名稱旁顯示其頭銜（例如 \"Arthas Jenkins\"）。"
L["Show Guild Name"] = "顯示公會名稱"
L["Show the player's guild name below their name."] = "在玩家名稱下方顯示其公會名稱。"
L["Show AFK Status"] = "顯示暫離狀態"
L["Show an <AFK> tag for away players."] = "為暫離的玩家顯示 <AFK> 標記。"
L["Friendly NPCs"] = "友方 NPC"
L["NPC titles fill in when you target or mouse over the NPC, or instantly with awesome_wotlk."] = "NPC 頭銜在選取或滑鼠停留該 NPC 時填入，使用 awesome_wotlk 則立即顯示。"
L["Headline Mode for NPCs"] = "NPC 僅名稱模式"
L["Apply headline mode to friendly NPCs."] = "對友方 NPC 套用僅名稱模式。"
L["Show NPC Title"] = "顯示 NPC 頭銜"
L["Show the NPC's title or occupation below its name (e.g. <General Supplies>)."] = "在 NPC 名稱下方顯示其頭銜或職業（例如 <雜貨商人>）。"
L["Show Full Plate on Target"] = "對目標顯示完整名牌"
L["When headline mode is active, show health, power and cast bars on your current target."] = "當標題模式啟用時，在當前目標上顯示生命、能量和施法條。"
L["Show Level Always"] = "始終顯示等級"
L["Show Level In Name When Targeted"] = "選中時在名字中顯示等級"
L["Show Level on Hover"] = "懸停時顯示等級"
L["Name Reaction Colors"] = "名字反應顏色"
L["Tint name text with the health bar reaction color (red/yellow/green/blue)."] = "以生命條反應顏色（紅/黃/綠/藍）為名字著色。"
L["Class Colors on Friendly Names"] = "友方玩家名字職業顏色"
L["Use class colors for friendly player name text."] = "對友方玩家名字使用職業顏色。需要在生命條中啟用 Friendly Class Colors 或 Party Class Colors。"
L["Class Colors on Enemy Names"] = "敵對玩家名字職業顏色"
L["Use class colors for enemy player name text."] = "對敵對玩家名字使用職業顏色。需要在生命條中啟用 Enemy Player Class Colors。"
L["Show Target Highlight"] = "目標高亮"
L["Highlight texture on the targeted nameplate."] = "目標名牌上的高亮紋理。"
L["Show Target Arrows"] = "目標箭頭"
L["Left/right arrows on the targeted nameplate."] = "目標名牌上的左右箭頭。"
L["Opacity"] = "透明度"
L["Disable Non-Target Fade"] = "停用非目標淡出"
L["Keep all nameplates fully opaque when targeting."] = "有目標時保持所有名牌完全不透明。"
L["Non-Target Opacity"] = "非目標透明度"
L["Opacity for non-target nameplates when fade is enabled (0.0-1.0)."] = "啟用淡出時非目標名牌的透明度 (0.0-1.0)。"
L["Show Debuffs"] = "顯示減益"
L["Show debuff icons on the target frame."] = "在目標框架上顯示減益圖標。"
L["Show debuff icons on the focus frame."] = "在焦點框架上顯示減益圖標。"
L["Show Buffs"] = "顯示增益"
L["Show buff icons on the target frame."] = "在目標框架上顯示增益圖標。"
L["Show buff icons on the focus frame."] = "在焦點框架上顯示增益圖標。"
L["Show Group Indicator"] = "顯示隊伍指示器"
L["Show the raid subgroup indicator (e.g. Group 2) above the player frame."] = "在玩家框架上方顯示團隊分組指示器（如第2組）。"
L["Class Color Name"] = "職業顏色名稱"
L["Center Name"] = "居中名稱"
L["Center the unit name above the health bar."] = "將單位名稱居中顯示在生命條上方。"
L["Max Debuff Icons"] = "最大減益圖示"
L["Only Show on Target & Focus"] = "僅在目標與專注目標時顯示"
L["Hide debuffs on every nameplate except your current target and focus."] = "隱藏除目前目標與專注目標以外所有名牌上的減益效果。"
L["Only My Debuffs"] = "僅我的減益"
L["Only show debuffs you applied yourself."] = "僅顯示你自己施加的減益效果。"
L["Debuff List Mode"] = "減益清單模式"
L["Choose whether the spell list below shows only listed debuffs or hides them."] = "選擇下方清單是僅顯示列出的減益還是隱藏它們。"
L["Whitelist"] = "白名單"
L["Blacklist"] = "黑名單"
L["Spell List"] = "法術清單"
L["Spell IDs separated by commas."] = "以逗號分隔的法術ID。"
L["Add Debuff by Name"] = "依名稱新增減益"
L["Add Debuff by ID"] = "依ID新增減益"
L["Export/Import Spell IDs"] = "匯出/匯入法術ID"
L["Enter a spell ID"] = "輸入法術ID"
L["Enter a spell name"] = "輸入法術名稱"
L["Paste spell IDs separated by commas."] = "貼上以逗號分隔的法術ID。"
L["Invalid spell name or ID"] = "無效的法術名稱或ID"
L["Click an entry to remove it."] = "點擊項目以移除。"
L["Click to remove."] = "點擊移除。"
L["Spell filter list is empty."] = "篩選清單為空。"
L["Spell ID: %d"] = "法術ID：%d"
L["Unknown"] = "未知"
L["Highlight Crowd Control"] = "高亮群體控制"
L["Adds a colored border to stuns, fears, polymorphs, silences, and other crowd control."] = "為暈眩、恐懼、變形、沉默及其他群體控制效果新增彩色邊框。"
L["Filtering"] = "篩選"
L["Priority Highlight"] = "優先高亮"
L["Power Bar"] = "能量條"
L["Debuffs"] = "減益"
L["Show Threat Glow"] = "顯示仇恨光暈"
L["Show Power Bar"] = "顯示能量條"
L["Show Power Bar Text"] = "顯示能量條文字"
L["Show numeric values (current / percent) on the power bar."] = "在能量條上顯示數值。"
L["Power Bar Players Only"] = "僅玩家能量條"
L["Hide mana/power on NPC nameplates when unit is known."] = "已知單位時在 NPC 名牌上隱藏法力/能量。"
L["On 3.3.5a, enemy cast bars appear on the targeted plate only."] = "在 3.3.5a 中，敵方施法條僅出現在目標名牌上。"
L["Show Cast Bar"] = "顯示施法條"
L["Show Cast Bars"] = "顯示施法條"
L["Show Spell Name"] = "顯示法術名稱"
L["Show the spell name text on the cast bar."] = "在施法條上顯示法術名稱文字。"
L["Spell Name Font Size"] = "法術名稱字體大小"
L["Spell Name Offset X"] = "法術名稱水平偏移"
L["Spell Name Offset Y"] = "法術名稱垂直偏移"
L["Cast Bar Height"] = "施法條高度"
L["Bar Stack Gap"] = "條形間距"
L["Vertical spacing between health, power, and cast bars (pixels)."] = "生命條、資源條與施法條之間的垂直間距（像素）。"
L["Show Party Cast Bars"] = "顯示隊伍施法條"
L["Show cast bars on party member nameplates even when not targeted."] = "即使未選中也在隊伍成員名牌上顯示施法條。"
L["Hide Pet Casts"] = "隱藏寵物施法條"
L["Hide Pet Casts Desc"] = "隱藏玩家寵物和守護者（水元素、鏡像等）的施法條。"
L["Show Enemy Player Cast Bars in PvP"] = "在PvP中顯示敵方玩家施法條"
L["In PvP, show enemy player cast bars without needing target or mouseover."] = "在PvP中，無需目標或滑鼠停留即可顯示敵方玩家施法條。"
L["Off-Target Cast Bars"] = "非目標施法條"
L["Off-Target Combat Log Mode"] = "非目標戰鬥紀錄模式"
L["Off-Target Mode Off"] = "關閉"
L["Off-Target Mode Hybrid"] = "混合（推薦）"
L["Off-Target Mode Aggressive"] = "積極"
L["Off-Target Mode When Safe"] = "安全時"
L["Off-Target Mode Off Desc"] = "不使用戰鬥紀錄顯示非目標施法。"
L["Off-Target Mode Hybrid Desc"] = "智慧預設模式：敵方和友方玩家使用積極技術（其名稱始終唯一），NPC使用更安全的安全時技術（其名稱可能重複）。結合兩者以在各種情況下獲得最佳效果。"
L["Off-Target Mode Aggressive Desc"] = "嘗試顯示更多非目標施法。假施法不會顯示為已打斷；施法條可能出現在錯誤的名牌上。"
L["Off-Target Mode When Safe Desc"] = "僅在匹配較可靠時顯示較少的非目標施法。"
L["Aggressive Filters"] = "積極篩選"
L["When Safe Filters"] = "安全時篩選"
L["Hostile Units Only"] = "僅敵對單位"
L["Safe Hostile Only Desc"] = "在戰鬥紀錄中略過友方和中立單位。"
L["Enemy Players Only"] = "僅敵方玩家"
L["Limit to enemy player cast bars. Useful in PvP."] = "將非目標施法限制為敵方玩家。"
L["Safe Enemy Players Only Desc"] = "僅敵方玩家。需要啟用僅敵對單位。"
L["Ignore Friendly Casts"] = "忽略友方施法"
L["Skip party, allies, and neutral units in the combat log."] = "在戰鬥紀錄中略過隊伍、盟友和中立單位。"
L["Enable Off-Target Detection"] = "啟用安全非目標偵測"
L["Enable Off-Target Detection Desc"] = "對曾設為 target、focus 或 mouseover 的單位顯示非目標施法條。"
L["Off-Target Aggressive Mode"] = "激進模式"
L["Off-Target Aggressive Mode Desc"] = "無需事先 target、focus 或 mouseover 即可顯示非目標施法。"
L["Off-Target Players Only"] = "僅玩家"
L["Off-Target Players Only Desc"] = "僅對玩家使用激進模式，世界怪物保持安全模式。"
L["Off-Target Aggressive Warning"] = "|cffffffff警告：|r\n- 若施法因移動或跳躍被取消（假施法），施法條仍會顯示至結束。\n- 同名怪物可能顯示在錯誤的姓名板上。"
L["Off-Target Players Only Warning"] = "|cffffffff警告：|r\n- 若施法因移動或跳躍被取消（假施法），施法條仍會顯示至結束。"
L["Icons"] = "圖示"
L["Show Raid Markers"] = "顯示團隊標記"
L["Beside Bar Layout"] = "置於血條旁"
L["Place the raid marker beside the bar (as with DragonUI debuffs). Useful with other aura addons."] = "將團隊標記放在血條旁（與 DragonUI 減益位置相同）。適合搭配其他光環插件。"
L["Show raid target markers (skull, cross, star, etc.) on nameplates."] = "在名牌上顯示團隊標記。"
L["Color Health Bar by Raid Marker"] = "依團隊標記為生命條上色"
L["Colors the health bar with the raid marker's color, on both allies and enemies."] = "用團隊標記的顏色為生命條上色，對友方與敵方皆適用。"
L["Show Elite Icon"] = "顯示精英圖示"
L["Show elite/rare dragon icon on nameplates."] = "在名牌上顯示精英/稀有圖示。"
L["Elite Icon Style"] = "精英圖示樣式"
L["Choose dragon or star style for elite and rare nameplate icons."] = "選擇精英和稀有名牌圖示的龍或星樣式。"
L["Dragon"] = "龍"
L["Star"] = "星"
L["Show Combo Points"] = "顯示連擊點"
L["Show combo points on the current target nameplate."] = "在目前目標名牌上顯示連擊點。"
L["Combo Points"] = "連擊點"
L["Anchor Position"] = "錨點位置"
L["Choose where the combo widget is anchored on the target nameplate health bar."] = "選擇連擊點元件在目標名牌生命條上的錨定位置。"
L["Scale of the combo widget (0.5 - 2.0)."] = "連擊點元件的縮放 (0.5 - 2.0)。"
L["Horizontal offset from the chosen anchor point. Negative moves left, positive moves right."] = "從所選錨點的水平偏移。負值向左移動，正值向右移動。"
L["Vertical offset from the chosen anchor point. Negative moves down, positive moves up."] = "從所選錨點的垂直偏移。負值向下移動，正值向上移動。"
L["Combos Per Row"] = "每行連擊數"
L["Maximum combo segments per row on custom-class resources (Reaper, Demonhunter, Ranger, ...). Rogue and Druid use a single combined icon and are not affected. Wrap to a second row when exceeded."] = "自訂職業資源（Reaper、Demonhunter、Ranger 等）每行最多顯示的連擊段數。盜賊和德魯伊使用單一組合圖示，不受影響。超出時換到第二行。"
L["Show Totem Icons"] = "顯示圖騰圖示"
L["Totem Icon Only"] = "僅顯示圖騰圖示"
L["Hide the totem's nameplate entirely and show only its icon."] = "完全隱藏圖騰的名牌，只顯示其圖示。"
L["Show totem icon on shaman totem nameplates."] = "在薩滿圖騰名牌上顯示圖騰圖示。"
L["Name and health percent font scale (1-10, default 2)."] = "名字和生命百分比字型縮放 (1-10，預設 2)。"
L["Size & Position"] = "大小和位置"
L["Health Bar"] = "生命條"
L["Health Bar Background"] = "生命條背景"
L["Choose the background texture used behind the health bar fill."] = "選擇生命條填充背後使用的背景紋理。"
L["Power Bar Background"] = "能量條背景"
L["Choose the background texture used behind the power bar fill."] = "選擇能量條填充背後使用的背景紋理。"
L["Black"] = "黑色"
L["Same as Castbar"] = "與施法條相同"
L["Overlay Name On Health Bar"] = "將名字疊加在生命條上"
L["Anchor the name, level, health percent, and elite icon centered on the health bar instead of above it."] = "將名字、等級、生命百分比和精英圖示居中固定在生命條上，而不是顯示在其上方。"
L["Overlay Vertical Offset"] = "垂直偏移"
L["Fine-tune the vertical position when 'Overlay Name On Health Bar' is enabled."] = "啟用「將名字疊加在生命條上」後，用此項微調垂直位置。"
L["Name Row Horizontal Padding"] = "名字行水平邊距"
L["Inset the name, level, and health percent from the left and right edges of the health bar. Does not affect the elite icon."] = "使名字、等級和生命百分比與生命條左右邊緣保持內縮間距。不影響精英圖示。"
L["Elite Icon Vertical Offset"] = "精英圖示垂直偏移"
L["Fine-tune the elite/rare icon's vertical position."] = "微調精英/稀有圖示的垂直位置。"
L["Name & Level"] = "名字和等級"
L["Target & Threat"] = "目標和仇恨"
L["Icons & Markers"] = "圖示和標記"
L["Power Bar — Players Only"] = "能量條 — 僅玩家"
L["Always show the unit level next to the name."] = "始終在名字旁邊顯示單位等級。"
L["White border glow on the current target nameplate."] = "目前目標名牌上的白色邊框光暈。"
L["Colored glow indicating aggro status (red = tanking, orange = losing, yellow = gaining)."] = "指示仇恨狀態的彩色光暈 (紅 = 坦克中，橙 = 失去中，黃 = 獲取中)。"
L["Tank Mode"] = "坦克模式"
L["Inverts threat colors for a tank: green means you hold aggro, red means you lost it."] = "為坦克反轉仇恨顏色：綠色表示你持有仇恨，紅色表示你已失去仇恨。"
L["DPS Mode"] = "DPS模式"
L["In combat, colors by threat for DPS: green = no aggro, yellow = transition, red = you have aggro."] = "戰鬥中依仇恨為DPS著色：綠色=無仇恨，黃色=過渡，紅色=你有仇恨。"
L["Keep all nameplates fully opaque when you have a target."] = "有目標時保持所有名牌不透明。"
L["Show enemy cast bar on the targeted nameplate."] = "在目標名牌上顯示敵方施法條。"
L["Name Font"] = "名字字型"

L["Behavior"] = "行為"
L["Allow Nameplate Overlap"] = "允許名牌重疊"
L["Allow plates to overlap instead of stacking."] = "允許名牌重疊而非堆疊。"
L["Addon Compatibility"] = "外掛相容性"
L["Enable this if you use an external nameplate addon (PlateBuffs, Crosshairs, ...) that isn't detecting DragonUI's nameplates correctly."] = "如果你使用的外部名牌外掛（如 PlateBuffs、Icicle 等）無法正確識別 DragonUI 的名牌，請啟用此選項。"
L["Nameplate Addon Compatibility"] = "名牌外掛相容性"
L["Stops overriding the nameplate's native transparency/visibility, which some external addons rely on to find their target. Non-target nameplates will dim like vanilla."] = "停止覆寫名牌的原生透明度／可見度，部分外部外掛依賴它來尋找目標。非目標名牌將如原版一樣變暗。"
L["Enable this if you use an addon (Icicle, ...) that attaches its own widgets to DragonUI's nameplate health bar."] = "如果你使用會把自身元件掛到 DragonUI 名牌原生生命條上的外掛（如 Icicle 等），請啟用此選項。"
L["Nameplate Health Bar Compatibility"] = "名牌生命條相容性"
L["Hides the native health bar by fading its individual textures instead of the whole bar, for addons that attach widgets directly to it."] = "透過淡出各條紋理而非整條生命條來隱藏原生生命條，供直接掛接元件的外掛使用。"
L["May increase CPU use with many visible nameplates."] = "可見名牌較多時可能增加 CPU 佔用。"
L["Depth Ordering"] = "深度排序"
L["Order overlapping nameplates by depth."] = "依深度排序重疊的名牌。"
L["(Requires Allow Nameplate Overlap.)"] = "（需要開啟「允許名牌重疊」）"
L["Retail-like Stacking"] = "Retail 式堆疊"
L["Simulates Retail's nameplate stacking for enemies."] = "模擬 Retail 敵人名牌堆疊。"
L["Collider Width"] = "碰撞體寬度"
L["Collider Height"] = "碰撞體高度"
L["Sets the width of the virtual collider centered on each nameplate used to detect overlaps."] = "每個名牌居中虛擬碰撞體的寬度。"
L["Sets the height of the virtual collider centered on each nameplate used to detect overlaps."] = "每個名牌居中虛擬碰撞體的高度。"
L["Vertical Offset"] = "垂直偏移"
L["Vertical offset baseline for Retail-like stacking."] = "Retail 式堆疊的垂直基準偏移。"
L["Freeze Mouseover"] = "凍結滑鼠指向"
L["Keeps the hovered nameplate fixed while stacking updates around it."] = "堆疊更新時保持滑鼠下的名牌固定。"
L["Disable in Open World"] = "在野外停用"
L["Only apply Retail-like stacking inside instances."] = "僅在副本內啟用 Retail 式堆疊。"
L["BG Healer Icon"] = "戰場治療圖示"
L["BattleGroundHealers Compatibility"] = "BattleGroundHealers 相容"
L["Keep BG healer marks attached to DragonUI nameplates."] = "將 BG 治療標記綁定到 DragonUI 名牌。"
L["Override BattleGroundHealers icon position on DragonUI nameplates."] = "覆寫 BGH 圖示在 DragonUI 名牌上的位置。"
L["This feature is available only when BattleGroundHealers is loaded."] = "僅在載入 BattleGroundHealers 時可用。"
L["Enable Test Mode"] = "啟用測試模式"
L["Enable manual marking for BattleGroundHealers compatibility checks."] = "手動標記以測試 BGH 相容。"
L["Mark Target"] = "標記目標"
L["Toggle BattleGroundHealers mark on your current target while test mode is enabled."] = "測試模式下切換目前目標的 BGH 標記。"
L["Totem Click Padding"] = "圖騰點擊邊距"
L["Extra clickable padding on totem nameplates (easier to click)."] = "圖騰名牌額外可點擊邊距。"
L["Clamp Target to Screen"] = "目標固定在螢幕內"
L["Keep the target nameplate visible at the top of the screen. Extends WorldFrame height when enabled."] = "讓目標名牌保持在畫面內可見。"
L["Clamp Bosses to Screen"] = "Boss 固定在螢幕內"
L["Keep boss nameplates visible at the top of the screen inside party and raid instances."] = "讓 Boss（骷髏等級 / ??）名牌保持在畫面內可見。"
L["Clamp Top Inset"] = "頂部邊距"
L["Distance below the top edge where clamped nameplates stop."] = "固定名牌停止的頂部下方距離。"
L["Bars"] = "條"
L["Clickbox"] = "點擊區域"
L["Clickbox Width Factor"] = "點擊區域寬度係數"
L["Clickbox Height Factor"] = "點擊區域高度係數"
L["Scales the nameplate clickbox relative to its original size. Recommended to change this setting while out of combat."] = "縮放點擊區域。建議脫離戰鬥修改。"
L["Show Clickbox"] = "顯示點擊區域"
L["Displays the box selection space (clickbox) of nameplates."] = "顯示名牌的點擊區域。"
L["Enemy Player Class Colors"] = "敵方玩家職業顏色"
L["Use class colors for enemy player nameplates."] = "敵方玩家名牌使用職業顏色。"
L["Show Party/Raid Cast Bars"] = "小隊/團隊施法條"
L["Show cast bars when the unit is known for sure: your target, focus, mouseover, arena enemies, or a group member's target."] = "單位可確定時顯示施法條。"
L["Also show cast bars on party and raid allies, even when you are not targeting them."] = "未選中時也顯示小隊/團隊盟友施法條。"
L["Background Plates Opacity"] = "背景名牌透明度"
L["Controls the opacity of non-target nameplates while fade is active (0.0 - 1.0)."] = "淡化時非目標名牌透明度 (0.0-1.0)。"
L["No Target: Full Opacity"] = "無目標：完全不透明"
L["When you have no target, show all nameplates at full opacity."] = "無目標時所有名牌完全不透明。"
L["Party/Raid: Full Opacity"] = "小隊/團隊：完全不透明"
L["Always show party and raid member nameplates at full opacity, regardless of target or fade settings. Does not affect pets or NPCs."] = "無論目標或淡化設定如何，始終以完全不透明顯示小隊/團隊成員名牌。不影響寵物或NPC。"
L["Behavior Mode"] = "行為模式"
L["Target"] = "目標"
L["Layout"] = "版面"
L["General"] = "一般"
L["Opacity for non-target nameplates when fade is enabled (0.1 - 1.0)."] = "淡化時非目標透明度 (0.0-1.0)。"

L["Use Target Opacity When No Target Exists"] = "Use Target Opacity When No Target Exists"
L["When no target exists, use target opacity instead of non-target opacity."] = "沒有目標時，使用目標透明度而不是非目標透明度。"

L["When no target exists, use full opacity instead of Non-Target Opacity."] = "When no target exists, use full opacity instead of Non-Target Opacity."

-- Nameplates：發佈版完整本地化
L["Allow native nameplates to overlap. Retail-like Stacking enables this automatically because its custom stacking algorithm requires overlap."] = "允許原生名牌互相重疊。仿正式服堆疊的自訂演算法需要重疊，因此會自動啟用此項。"
L["Anchor"] = "錨點"
L["Color the glow and health bar by threat status (red = tanking, orange = losing, yellow = gaining)."] = "依仇恨狀態為發光和生命條著色（紅色=正在承受，橙色=正在失去，黃色=正在獲得）。"
L["Comma-separated, exact totem names (as shown in-game) that should never get a totem icon and render as a normal nameplate instead."] = "輸入以逗號分隔的遊戲內精確圖騰名稱；這些圖騰不顯示圖示，而使用一般名牌。"
L["Keep hostile boss and world-boss nameplates visible at the top of the screen wherever they appear."] = "無論在何處出現，都讓敵對首領和世界首領名牌保持在畫面頂端可見。"
L["Only apply Retail-like stacking inside party and raid instances. It remains disabled in the open world, battlegrounds, and arenas."] = "僅在地城與團隊副本中套用仿正式服堆疊；在開放世界、戰場和競技場中維持停用。"
L["Scales the nameplate clickbox relative to its original size. Changes made during combat are applied when combat ends."] = "依原始尺寸縮放名牌點擊區域。戰鬥中所做的變更會在戰鬥結束後套用。"
L["Show icons for recognized shaman totems. DragonUI uses localized spell names and automatically learns your own active totems."] = "為已識別的薩滿圖騰顯示圖示。DragonUI 使用本地化法術名稱，並自動學習你目前啟用的圖騰。"
L["Show remaining life on your own totems (requires their icon to be known)."] = "顯示自己圖騰的剩餘時間（必須已識別其圖示）。"
L["Show Totem Life Timer"] = "顯示圖騰剩餘時間"
L["Totems Without Icon"] = "不顯示圖示的圖騰"
L["Choose where the debuff cooldown text is anchored on the icon."] = "選擇減益剩餘時間文字在圖示上的錨定位置。"
L["Choose where the totem icon is anchored around the nameplate."] = "選擇圖騰圖示在名牌周圍的錨定位置。"
L["Debuff Cooldown Font Size"] = "減益剩餘時間字型大小"
L["Debuff Cooldown Text Position"] = "減益剩餘時間文字位置"
L["Font size for debuff remaining time text."] = "減益剩餘時間文字的字型大小。"
L["Level Format"] = "等級格式"
L["Show Debuff Cooldown Text"] = "顯示減益剩餘時間"
L["Show remaining debuff time on each debuff icon."] = "在每個減益圖示上顯示剩餘時間。"
L["Size of debuff icons on nameplates."] = "名牌上減益圖示的大小。"
L["Totem Icon Position"] = "圖騰圖示位置"

-- Version Check
L["Version Check"] = "版本檢查"
L["Broadcast and detect addon version updates across group members."] = "檢測隊伍成員間插件版本更新，通過廣播和接收版本資訊。"

-- Search

L["Search settings..."] = "搜尋設定..."
L["Type to find a setting"] = "輸入以尋找設定"
L["No settings match '%s'."] = "沒有與「%s」相符的設定。"
L["Showing top %d results. Type at least 3 characters for the full list."] = "僅顯示前 %d 筆結果。輸入至少 3 個字元可查看完整清單。"

-- Aura Borders: border style dropdown
L["Border Style"] = "邊框樣式"
L["Rounded"] = "圓角"
L["Square"] = "方形"

-- Range Indicator colors
L["Out of Range Color"] = "超出範圍顏色"
L["Not Enough Mana Color"] = "法力不足顏色"

-- ============================================================================
-- Added missing translations
-- ============================================================================
L["Aura Borders"] = "光環邊框"
L["Enable Aura Borders"] = "啟用光環邊框"
L["Show modern borders around buff and debuff icons."] = "在增益與減益圖示周圍顯示現代風格邊框。"
L["Buff Border Color"] = "增益邊框顏色"
L["Reset Debuff Position"] = "重設減益位置"
L["Debuff position reset."] = "減益位置已重設。"
L["Debuffs detached - positioned freely via Editor Mode"] = "減益已分離 - 可透過編輯模式自由定位"
L["Debuffs attached - follow buff row"] = "減益已附著 - 跟隨增益列"
L["Opacity of tabs, buttons and chat background when not hovered. 0 = hidden, 1 = always visible."] = "未懸停時分頁、按鈕與聊天背景的不透明度。0 = 隱藏，1 = 始終顯示。"
L["Lock Toggle Hotkey"] = "鎖定切換快捷鍵"
L["Choose the modifier + mouse button used to lock or unlock a bag slot while hovering it."] = "選擇在懸停背包格子時用於鎖定或解鎖的修飾鍵 + 滑鼠按鍵。"
L["Use /sortlock to lock or unlock the currently hovered slot from chat."] = "使用 /sortlock 從聊天視窗鎖定或解鎖目前懸停的格子。"
L["Alt + Left Click"] = "Alt + 左鍵單擊"
L["Ctrl + Left Click"] = "Ctrl + 左鍵單擊"
L["Shift + Left Click"] = "Shift + 左鍵單擊"
L["Alt + Right Click"] = "Alt + 右鍵單擊"
L["Ctrl + Right Click"] = "Ctrl + 右鍵單擊"
L["Shift + Right Click"] = "Shift + 右鍵單擊"
L["Alt + Middle Click"] = "Alt + 中鍵單擊"
L["Ctrl + Middle Click"] = "Ctrl + 中鍵單擊"
L["Shift + Middle Click"] = "Shift + 中鍵單擊"
L["Extra Bar Scale"] = "額外動作條縮放"
L["Bar"] = "動作條"
L["Hide the main bar while in combat."] = "戰鬥中隱藏主動作條。"
L["Fade the quest tracker until you hover over it."] = "在你懸停之前淡出任務追蹤器。"
L["Fade the quest tracker until you enter combat."] = "在你進入戰鬥之前淡出任務追蹤器。"
L["Hide the quest tracker while in combat."] = "戰鬥中隱藏任務追蹤器。"
L["Fade the XP and Reputation bars until you hover over them."] = "在你懸停之前淡出經驗值與聲望條。"
L["Fade the XP and Reputation bars until you enter combat."] = "在你進入戰鬥之前淡出經驗值與聲望條。"
L["Hide the XP and Reputation bars while in combat."] = "戰鬥中隱藏經驗值與聲望條。"
L["Fade the player frame until you hover over it."] = "在你懸停之前淡出玩家框架。"
L["Fade the player frame until you enter combat."] = "在你進入戰鬥之前淡出玩家框架。"
L["Also fades the Target of Target and target cast bar, attached or not."] = "同時淡出目標的目標與目標施法條，無論是否附著。"
L["Fade the target frame group until you hover over it."] = "在你懸停之前淡出目標框架群組。"
L["Fade the target frame group until you enter combat."] = "在你進入戰鬥之前淡出目標框架群組。"
L["Also fades the Target of Focus and focus cast bar, attached or not."] = "同時淡出專注目標的目標與專注目標施法條，無論是否附著。"
L["Fade the focus frame group until you hover over it."] = "在你懸停之前淡出專注目標框架群組。"
L["Fade the focus frame group until you enter combat."] = "在你進入戰鬥之前淡出專注目標框架群組。"
L["Fade the pet frame until you hover over it."] = "在你懸停之前淡出寵物框架。"
L["Fade the pet frame until you enter combat."] = "在你進入戰鬥之前淡出寵物框架。"
L["Fade party frames until you hover over them."] = "在你懸停之前淡出隊伍框架。"
L["Fade party frames until you enter combat."] = "在你進入戰鬥之前淡出隊伍框架。"
L["Fade boss frames until you hover over them."] = "在你懸停之前淡出首領框架。"
L["Fade boss frames until you enter combat."] = "在你進入戰鬥之前淡出首領框架。"
L["Boss"] = "首領"
L["Fade the stance bar until you hover over it."] = "在你懸停之前淡出姿態條。"
L["Fade the stance bar until you enter combat."] = "在你進入戰鬥之前淡出姿態條。"
L["Fade the pet bar until you hover over it."] = "在你懸停之前淡出寵物動作條。"
L["Fade the pet bar until you enter combat."] = "在你進入戰鬥之前淡出寵物動作條。"
L["Fade the totem bar until you hover over it."] = "在你懸停之前淡出圖騰條。"
L["Fade the totem bar until you enter combat."] = "在你進入戰鬥之前淡出圖騰條。"
L["Fade the minimap until you hover over it."] = "在你懸停之前淡出小地圖。"
L["Fade the minimap until you enter combat."] = "在你進入戰鬥之前淡出小地圖。"
L["Experimental:"] = "實驗性："
L["Fading the minimap can occasionally cause tracking icons to stop updating until you /reload. Use at your own risk."] = "淡出小地圖偶爾會導致追蹤圖示在你執行 /reload 之前停止更新。使用風險自負。"
L["Has no effect while SexyMap controls the minimap (SexyMap or Hybrid mode)."] = "當 SexyMap 控制小地圖時無效（SexyMap 或混合模式）。"
L["Hide in Combat"] = "戰鬥中隱藏"
L["Hide the minimap while in combat."] = "戰鬥中隱藏小地圖。"
L["Show Stance Bar"] = "顯示姿態條"
L["Show Totem Bar"] = "顯示圖騰條"
L["Configure how DragonUI notifies you when friends come online or go offline."] = "設定當好友上線或離線時 DragonUI 通知你的方式。"
L["Enable BNet Toast"] = "啟用 BNet 通知"
L["Display Battle.net friend online/offline notifications via toast popup and/or chat messages."] = "透過彈出通知和/或聊天訊息顯示 Battle.net 好友上線/離線通知。"
L["Notification Type"] = "通知類型"
L["Guild Notifications"] = "公會通知"
L["Show notifications for guild members coming online or going offline. Turn off to only receive friend notifications."] = "當公會成員上線或離線時顯示通知。關閉後僅接收好友通知。"
L["Show Toast Popup"] = "顯示彈出通知"
L["Display the Battle.net toast frame when a friend comes online or goes offline."] = "當好友上線或離線時顯示 Battle.net 通知框。"
L["Show Chat Notification"] = "顯示聊天通知"
L["Display a chat message when a friend comes online or goes offline."] = "當好友上線或離線時顯示聊天訊息。"
L["Share to:"] = "分享至："
L["Whisper"] = "密語"
L["Raid"] = "團隊"
L["Guild"] = "公會"
L["Share"] = "分享"
L["Enter a target player name for whisper."] = "輸入要密語的目標玩家名稱。"
L["Profile shared with"] = "設定檔已分享給"
L["Uses the combat log to guess casts on units you are not targeting. Less accurate than target, focus, mouseover, and arena enemies above."] = "使用戰鬥記錄來推測你未選定單位的施法。精確度低於上方的目標、專注目標、滑鼠懸停與競技場敵人。"
L["Sync From Native Cast Bar"] = "從原生施法條同步"
L["On supported clients, use the game's built-in nameplate cast bar for better timing. Safe to leave enabled."] = "在受支援的客戶端上，使用遊戲內建的名牌施法條以獲得更佳時機。可安全保持啟用。"
L["Native Progress Sync for Off-Target Casts"] = "非目標施法的原生進度同步"
L["When available, off-target combat-log castbars sync their progress from Blizzard's native castbar."] = "如果可用，非目標戰鬥記錄施法條會從暴雪原生施法條同步進度。"
L["showVKeyCastbar Mode"] = "showVKeyCastbar 模式"
L["auto: keep client value. force_on/off: DragonUI sets showVKeyCastbar while nameplates module is active."] = "auto：保留客戶端值。force_on/off：在名牌模組啟用時由 DragonUI 設定 showVKeyCastbar。"
L["Auto (Do Not Force)"] = "自動（不強制）"
L["Force On"] = "強制開啟"
L["Force Off"] = "強制關閉"
L["DragonUI (Custom)"] = "DragonUI（自訂）"
L["Transmog Collector"] = "幻化收集器"
L["Enable Transmog Collector"] = "啟用幻化收集器"
L["Automatically collect transmog appearances when looting new items. Works with Ascension's Ctrl+Alt+Click appearance system."] = "在拾取新物品時自動收集幻化外觀。與 Ascension 的 Ctrl+Alt+單擊外觀系統相容。"
L["On loot, auto-collects appearances for items you haven't learned yet."] = "拾取時自動收集你尚未學會的物品外觀。"
L["Attack Bar"] = "攻擊條"
L["Shows swing timers for your main hand, off hand, ranged attacks, and enemy target melee swings."] = "顯示你的主手、副手、遠程攻擊以及敵方目標近戰揮擊的揮擊計時器。"
L["Enable Attack Bar"] = "啟用攻擊條"
L["Show swing timer bars for melee and ranged attacks."] = "顯示近戰與遠程攻擊的揮擊計時器條。"
L["Main Hand Bar"] = "主手條"
L["Show the main hand melee swing timer."] = "顯示主手近戰揮擊計時器。"
L["Off Hand Bar"] = "副手條"
L["Show the off hand melee swing timer (dual-wield)."] = "顯示副手近戰揮擊計時器（雙持）。"
L["Ranged Bar"] = "遠程條"
L["Show the ranged attack timer (Hunter shots, Throw, Aimed Shot)."] = "顯示遠程攻擊計時器（獵人射擊、投擲、瞄準射擊）。"
L["Enemy Bar"] = "敵人條"
L["Show the enemy target melee swing timer."] = "顯示敵方目標近戰揮擊計時器。"
L["Show Timer"] = "顯示計時器"
L["Show the countdown timer text on bars."] = "在條上顯示倒數計時文字。"
L["Show Info"] = "顯示資訊"
L["Show spell name, damage range, and time remaining text."] = "顯示法術名稱、傷害範圍與剩餘時間文字。"
L["Choose the border style for the swing timer bars."] = "選擇揮擊計時器條的邊框樣式。"
L["Standard"] = "標準"
L["Thin"] = "細"
L["Scale of the swing timer bars."] = "揮擊計時器條的縮放。"
L["Swing Timer Module"] = "揮擊計時器模組"
L["Shows main hand, off hand, ranged, and enemy target swing timers."] = "顯示主手、副手、遠程與敵方目標的揮擊計時器。"
-- Quest nameplate icons (loot providers + Questie coexistence)
L["Resolve By Name"] = "依名稱識別"
L["Match plate names to your active objectives so icons show on every plate without awesome_wotlk. Kill objectives work on their own; loot needs a quest addon below."] = "將姓名板名稱與你的進行中目標比對，從而在沒有 awesome_wotlk 的情況下在每個姓名板上顯示圖示。擊殺目標可獨立運作；拾取需要下方的任務插件。"
L["Loot Database"] = "掉落資料庫"
L["Which quest addon supplies loot data (which mob drops a quest item). Auto picks the best loaded one."] = "由哪個任務插件提供掉落資料（哪個怪物掉落任務物品）。自動會選擇已載入的最佳插件。"
L["Auto"] = "自動"
L["Icons With Questie"] = "與 Questie 的圖示"
L["Who draws quest icons on plates when Questie is loaded with its own nameplate icons on. DragonUI disables Questie's (needs reload); Questie hides DragonUI's."] = "當 Questie 已載入並開啟其自身姓名板圖示時，由誰在姓名板上繪製任務圖示。DragonUI 會停用 Questie 的（需重載）；Questie 會隱藏 DragonUI 的。"
L["Ask"] = "詢問"

-- 拾取視窗
L["Loot Window"] = "拾取視窗"
L["Configure the DragonUI loot window."] = "設定 DragonUI 拾取視窗。"
L["Enable Loot Window"] = "啟用拾取視窗"
L["Apply the DragonUI skin to the Blizzard loot window."] = "將 DragonUI 外觀套用至暴雪拾取視窗。"
L["Open at Cursor"] = "在游標位置開啟"
L["Open the loot window at the cursor instead of its saved position."] = "在游標位置而非儲存的位置開啟拾取視窗。"
L["Disable Open at Cursor, then drag the loot window to save its position."] = "停用在游標位置開啟，然後拖曳拾取視窗以儲存其位置。"
L["Animate Loot Reflow"] = "播放拾取重排動畫"
L["Smoothly close gaps and resize the loot window after collecting items."] = "拾取物品後平滑填補空隙並調整拾取視窗大小。"
L["Reset Loot Window Position"] = "重設拾取視窗位置"
L["Clear the saved position. The Blizzard default will be used next time you open the loot window."] = "清除儲存的位置。下次開啟拾取視窗時將使用暴雪預設位置。"

-- Item Level
L["Item Level"] = "物品等級"
L["Enable Item Level"] = "啟用物品等級"
L["Show the item level on gear icons across bags, bank, character panel and more."] = "在背包、銀行、角色面板等介面的裝備圖示上顯示物品等級。"
L["Font"] = "字型"
L["Default (Arial Narrow)"] = "預設 (Arial Narrow)"
L["Size of the item level number on the icon."] = "圖示上物品等級數字的大小。"
L["Outline"] = "外框"
L["Thickness of the black outline. WoW 3.3.5a has no real bold, so a thicker outline is what makes the number look heavier."] = "黑色外框的粗細。WoW 3.3.5a 沒有真正的粗體，因此只有加粗外框才能讓數字看起來更醒目。"
L["Thick"] = "粗"
L["Vertical position of the item level number on the icon."] = "圖示上物品等級數字的垂直位置。"
L["Average Item Level"] = "平均物品等級"
L["Show the average item level of equipped gear on the character and inspect panels."] = "在角色與觀察面板上顯示已裝備物品的平均物品等級。"
L["Show in Tooltip"] = "在提示中顯示"
L["Also enable Blizzard's own item level line in item tooltips."] = "同時啟用暴雪內建的物品等級提示行。"
L["Choose where the number appears:"] = "選擇數字顯示的位置："
L["Guild Bank"] = "公會銀行"
L["Character Panel"] = "角色面板"
L["Inspect"] = "觀察"
L["Merchant"] = "商人"
L["Trade"] = "交易"
L["Loot"] = "拾取"
L["Mail"] = "郵件"
L["Auction House"] = "拍賣場"


-- Alt Gold
L["Alt Gold"] = "其他角色金幣"
L["Enable Alt Gold"] = "啟用其他角色金幣"
L["Hover the coins in your bags to list the gold of every character that has logged in with DragonUI."] = "將滑鼠移到揹包的金幣上，即可列出所有使用 DragonUI 登入過的角色的金幣。"
L["Show a tooltip with your other characters' gold when hovering the money in bags."] = "將滑鼠移到揹包金錢上時，顯示包含其他角色金幣的提示。"
L["Show All Realms"] = "顯示所有伺服器"
L["List characters from every realm instead of only the realm you are playing on."] = "列出所有伺服器的角色，而不僅是當前伺服器。"

-- Nameplate aura section
L["Ally Aura List"] = "友方光環列表"
L["Always Highlight These Spells"] = "總是突顯這些法術"
L["Always Show Crowd Control"] = "總是顯示控場效果"
L["Always Show Defensive Cooldowns"] = "總是顯示防禦技能"
L["Always Show Others' Crowd Control"] = "總是顯示他人的控場效果"
L["An ally carries dozens of auras, so only what you list here is shown."] = "一名隊友身上有數十個光環，因此只顯示這裡列出的內容。"
L["Any other buff has to be listed here, or an ally in a raid fills the screen."] = "其他增益必須在此列出，否則團隊中的隊友會佔滿螢幕。"
L["Aura Horizontal Offset"] = "水平偏移"
L["Aura Icon Size"] = "圖示大小"
L["Aura Row"] = "光環列"
L["Aura Vertical Offset"] = "垂直偏移"
L["Blizzard's dispel colors, editable below."] = "暴雪的驅散顏色，可在下方修改。"
L["Border Colors"] = "邊框顏色"
L["Buff List"] = "增益列表"
L["Buffs and debuffs share these slots."] = "增益與減益共用這些位置。"
L["Buffs share the row with debuffs, ordered by priority."] = "增益與減益共用一列，依優先順序排列。"
L["Colored Aura Borders"] = "彩色光環邊框"
L["Cooldown Font Size"] = "剩餘時間字型大小"
L["Cooldown Swipe Style"] = "冷卻掃描樣式"
L["Cooldown Text Position"] = "剩餘時間位置"
L["Crowd Control"] = "控場效果"
L["Crowd Control + List"] = "控場效果 + 列表"
L["Curse"] = "詛咒"
L["Debuff List"] = "減益列表"
L["Defensive Buff List"] = "防禦性增益列表"
L["Disease"] = "疾病"
L["Enemy Buffs"] = "敵方增益"
L["Enemy crowd control matters no matter who cast it."] = "無論由誰施放，控場效果都很重要。"
L["Enrage"] = "激怒"
L["Every debuff, not just crowd control. Useful for spotting what to dispel."] = "所有減益，不只是控場效果。方便查看需要驅散什麼。"
L["Friendly Plates"] = "友方名條"
L["Hide auras on every nameplate except your target and focus."] = "除了目標與焦點外，隱藏所有名條上的光環。"
L["Highlight Size"] = "突顯大小"
L["Highlighted Auras"] = "突顯的光環"
L["Highlighted auras draw larger than the rest."] = "突顯的光環會比其他光環更大。"
L["Icon Order"] = "圖示順序"
L["Importance"] = "重要性"
L["Importance: crowd control first, then enemy defensives, then your own debuffs."] = "重要性：先是控場效果，接著是敵方防禦技能，再是你自己的減益。"
L["Magic"] = "魔法"
L["Max Aura Icons"] = "最大光環圖示數"
L["Modern Icon Border"] = "現代圖示邊框"
L["Modern Icon Border Debuff Desc"] = "以 DragonUI 快捷列邊框為光環圖示加框。"
L["Multiplier over the base icon size."] = "相對基礎大小的倍數。"
L["No Dispel Type"] = "無驅散類型"
L["Nothing"] = "無"
L["Order Icons By"] = "圖示排序方式"
L["Poison"] = "中毒"
L["Purgeable & Defensive"] = "可驅散與防禦性"
L["Radial sweep over the icon as it runs out."] = "圖示隨時間流逝的環形掃描。"
L["Separate Color for Crowd Control"] = "控場效果使用獨立顏色"
L["Show All Debuffs"] = "顯示所有減益"
L["Show Auras"] = "顯示光環"
L["Show Auras on Allies"] = "在隊友身上顯示光環"
L["Show Cooldown Swipe"] = "顯示冷卻掃描"
L["Show Cooldown Text"] = "顯示剩餘時間"
L["Show Enemy Buffs"] = "顯示敵方增益"
L["Show buffs and debuffs above nameplates."] = "在名條上方顯示增益與減益。"
L["Shows buffs you can dispel or steal, plus the defensive cooldowns listed here."] = "顯示你可以驅散或偷取的增益，以及下方列出的防禦技能。"
L["Spell List Only"] = "僅法術列表"
L["Stuns, fears and polymorphs on an ally, listed or not."] = "隊友身上的昏迷、恐懼與變形，無論是否在列表中。"
L["Stuns, fears, roots and polymorphs are detected automatically. Add anything the game does not flag."] = "昏迷、恐懼、定身與變形會自動辨識。請在此加入遊戲未標記的法術。"
L["Time Remaining"] = "剩餘時間"
L["Time Remaining: whatever expires soonest goes first."] = "剩餘時間：最先結束的排在最前。"
L["Time left on each icon."] = "每個圖示上的剩餘時間。"
L["Timers"] = "計時"
L["Uses the defensive buff list from the Enemy Buffs section."] = "使用敵方增益區塊中的防禦性增益列表。"
L["What Gets Highlighted"] = "突顯哪些光環"
L["Which Buffs"] = "哪些增益"
L["An ally carries dozens of auras, so you pick below what earns a slot."] = "一名隊友身上有數十個光環，因此在下方選擇哪些值得顯示。"
L["Debuffs on the Ally"] = "隊友身上的減益"
L["Buffs the Ally Carries"] = "隊友擁有的增益"
L["Also Show These Spells"] = "另外顯示這些法術"
L["Listed spells always show, buff or debuff, on top of everything above."] = "列表中的法術總是顯示，無論增益或減益，並疊加在以上設定之上。"

-- Added by DragonUI (missing translations)
L["Aura Borders"] = "光環邊框"
L["Enable Aura Borders"] = "啟用光環邊框"
L["Show modern borders around buff and debuff icons."] = "在增益與減益圖示周圍顯示現代風格邊框。"
L["Buff Border Color"] = "增益邊框顏色"
L["Reset Debuff Position"] = "重置減益位置"
L["Debuff position reset."] = "已重置減益位置。"
L["Debuffs detached - positioned freely via Editor Mode"] = "減益已分離 - 可透過編輯模式自由擺放"
L["Debuffs attached - follow buff row"] = "減益已附著 - 跟隨增益列"
L["Opacity of tabs, buttons and chat background when not hovered. 0 = hidden, 1 = always visible."] = "未將滑鼠移至其上時，分頁、按鈕與聊天背景的不透明度。0 = 隱藏，1 = 永遠顯示。"
L["Lock Toggle Hotkey"] = "鎖定切換快捷鍵"
L["Choose the modifier + mouse button used to lock or unlock a bag slot while hovering it."] = "選擇用來鎖定或解除鎖定滑鼠所指背包格子的組合鍵與滑鼠按鍵。"
L["Use /sortlock to lock or unlock the currently hovered slot from chat."] = "可在聊天框使用 /sortlock 鎖定或解除鎖定目前滑鼠所指的格子。"
L["Reverse Stack Order"] = "反轉堆疊順序"
L["Stack sorted items from the end of each bag so empty slots stay at the top."] = "從每個背包的末端開始堆放已整理的物品，使空格保留在上方。"
L["Alt + Left Click"] = "Alt + 左鍵點擊"
L["Ctrl + Left Click"] = "Ctrl + 左鍵點擊"
L["Shift + Left Click"] = "Shift + 左鍵點擊"
L["Alt + Right Click"] = "Alt + 右鍵點擊"
L["Ctrl + Right Click"] = "Ctrl + 右鍵點擊"
L["Shift + Right Click"] = "Shift + 右鍵點擊"
L["Alt + Middle Click"] = "Alt + 中鍵點擊"
L["Ctrl + Middle Click"] = "Ctrl + 中鍵點擊"
L["Shift + Middle Click"] = "Shift + 中鍵點擊"

-- Added by DragonUI (missing translations)
L["Extra Bar Scale"] = "額外動作條縮放"
L["Bar"] = "動作條"

-- Added by DragonUI (missing translations)
L["Hide the main bar while in combat."] = "戰鬥中隱藏主動作條。"
L["Fade the quest tracker until you hover over it."] = "在滑鼠移至其上之前淡出任務追蹤器。"
L["Fade the quest tracker until you enter combat."] = "在進入戰鬥之前淡出任務追蹤器。"
L["Hide the quest tracker while in combat."] = "戰鬥中隱藏任務追蹤器。"
L["Fade the XP and Reputation bars until you hover over them."] = "在滑鼠移至其上之前淡出經驗條與聲望條。"
L["Fade the XP and Reputation bars until you enter combat."] = "在進入戰鬥之前淡出經驗條與聲望條。"
L["Hide the XP and Reputation bars while in combat."] = "戰鬥中隱藏經驗條與聲望條。"
L["Fade the player frame until you hover over it."] = "在滑鼠移至其上之前淡出玩家框體。"
L["Fade the player frame until you enter combat."] = "在進入戰鬥之前淡出玩家框體。"
L["Also fades the Target of Target and target cast bar, attached or not."] = "同時淡出目標的目標與目標施法條，無論是否附著。"
L["Fade the target frame group until you hover over it."] = "在滑鼠移至其上之前淡出目標框體群組。"
L["Fade the target frame group until you enter combat."] = "在進入戰鬥之前淡出目標框體群組。"
L["Also fades the Target of Focus and focus cast bar, attached or not."] = "同時淡出專注目標的目標與專注目標施法條，無論是否附著。"
L["Fade the focus frame group until you hover over it."] = "在滑鼠移至其上之前淡出專注目標框體群組。"
L["Fade the focus frame group until you enter combat."] = "在進入戰鬥之前淡出專注目標框體群組。"
L["Fade the pet frame until you hover over it."] = "在滑鼠移至其上之前淡出寵物框體。"
L["Fade the pet frame until you enter combat."] = "在進入戰鬥之前淡出寵物框體。"
L["Fade party frames until you hover over them."] = "在滑鼠移至其上之前淡出隊伍框體。"
L["Fade party frames until you enter combat."] = "在進入戰鬥之前淡出隊伍框體。"
L["Fade boss frames until you hover over them."] = "在滑鼠移至其上之前淡出首領框體。"
L["Fade boss frames until you enter combat."] = "在進入戰鬥之前淡出首領框體。"
L["Boss"] = "首領"
L["Fade the stance bar until you hover over it."] = "在滑鼠移至其上之前淡出姿態條。"
L["Fade the stance bar until you enter combat."] = "在進入戰鬥之前淡出姿態條。"
L["Fade the pet bar until you hover over it."] = "在滑鼠移至其上之前淡出寵物動作條。"
L["Fade the pet bar until you enter combat."] = "在進入戰鬥之前淡出寵物動作條。"
L["Fade the totem bar until you hover over it."] = "在滑鼠移至其上之前淡出圖騰條。"
L["Fade the totem bar until you enter combat."] = "在進入戰鬥之前淡出圖騰條。"
L["Fade the minimap until you hover over it."] = "在滑鼠移至其上之前淡出小地圖。"
L["Fade the minimap until you enter combat."] = "在進入戰鬥之前淡出小地圖。"
L["Experimental:"] = "實驗性："
L["Fading the minimap can occasionally cause tracking icons to stop updating until you /reload. Use at your own risk."] = "淡出小地圖偶爾會導致追蹤圖示在 /reload 之前停止更新。請自行斟酌使用。"
L["Has no effect while SexyMap controls the minimap (SexyMap or Hybrid mode)."] = "當 SexyMap 控制小地圖時無效（SexyMap 或混合模式）。"
L["Hide in Combat"] = "戰鬥中隱藏"
L["Hide the minimap while in combat."] = "戰鬥中隱藏小地圖。"

-- Added by DragonUI (missing translations)
L["Modern Icon Border Desc"] = "使用 DragonUI 動作條邊框取代傳統圓環來包覆法術圖示。"

-- Added by DragonUI (missing translations)
L["Total icons in the row. Buffs and debuffs share these slots, lowest priority drops first."] = "該列的圖示總數。增益與減益共用這些格位，優先度最低者最先捨棄。"
L["Lets stuns, fears and polymorphs from anyone through the Only My Debuffs filter. Enemy crowd control matters no matter who cast it."] = "即使套用「僅我的減益」篩選，仍顯示任何人施放的暈眩、恐懼與變形。敵方控制效果無論由誰施放都很重要。"
L["Show buffs on enemy plates in the same row as debuffs. They compete for the same slots, ordered by priority."] = "在敵方名條上將增益與減益顯示於同一列。兩者爭奪相同格位，並依優先度排序。"
L["Purgeable shows what you can spellsteal or dispel, plus defensive cooldowns worth waiting out. No list to maintain."] = "「可驅散」會顯示你能偷取或驅散的效果，以及值得等待結束的防禦性冷卻。無需維護清單。"
L["Only My Buffs"] = "僅我的增益"
L["Only show buffs you applied yourself."] = "僅顯示由你自己施加的增益。"
L["Priority"] = "優先度"
L["Sort Order"] = "排序方式"
L["Priority puts your crowd control first, then anyone's crowd control, enemy defensives, your debuffs and the rest. Chronological sorts by time left only."] = "「優先度」會先排你的控制效果，接著是任何人的控制、敵方防禦、你的減益，其餘在後。「依時間」僅依剩餘時間排序。"
L["Scale Icons by Priority"] = "依優先度縮放圖示"
L["Draws higher-priority auras larger. Icons stay bottom-aligned, so the big ones grow upward without shifting the row."] = "將優先度較高的光環畫得更大。圖示維持底部對齊，因此大圖示向上延伸而不會推移整列。"
L["Tints the icon frame by category: crowd control, enemy defensives, purgeable buffs, and dispel type for everything else."] = "依類別為圖示外框上色：控制效果、敵方防禦、可驅散增益，其餘則依驅散類型。"
L["Color by Dispel Type"] = "依驅散類型上色"
L["Show All Debuffs on Allies"] = "顯示友方的所有減益"
L["Decides which icons come first, and which drop off when there is no room. Importance puts crowd control first, then enemy defensives, then your own debuffs."] = "決定哪些圖示排在前面，以及空間不足時捨棄哪些。「重要度」優先控制效果，其次敵方防禦，再來是你自己的減益。"
L["Extra Crowd Control Spells"] = "額外控制法術"
L["Show stuns, fears and polymorphs on allies even when they are not in the list."] = "即使不在清單中，仍顯示友方身上的暈眩、恐懼與變形。"
L["An ally in a raid carries dozens of auras, so nothing is shown unless you ask for it below."] = "團隊中的友方身上有數十個光環，因此除非你於下方指定，否則不顯示任何內容。"
L["Highlighted auras draw larger than the rest. Everything else uses the base icon size."] = "被強調的光環會畫得比其餘更大，其餘則使用基本圖示尺寸。"
L["Same rule as the Blizzard target frame: each debuff takes the colour of its dispel type, and one without a type uses None."] = "與暴雪目標框體相同的規則：每個減益採用其驅散類型的顏色，沒有類型者使用「無」。"
L["Crowd control uses its own colour instead of its dispel type."] = "控制效果使用自己的顏色，而非驅散類型的顏色。"
L["Order: your crowd control, others' crowd control, enemy defensives, your debuffs, purgeable buffs, others' debuffs."] = "順序：你的控制、他人的控制、敵方防禦、你的減益、可驅散增益、他人的減益。"
L["Higher priority draws larger."] = "優先度越高，畫得越大。"
L["Tint the icon frame by category."] = "依類別為圖示外框上色。"
L["Enemy Defensives"] = "敵方防禦"
L["Purgeable"] = "可驅散"
L["Everything else uses its Magic, Curse, Disease or Poison color."] = "其餘皆使用其魔法、詛咒、疾病或中毒的顏色。"
L["Purgeable means Magic or Enrage, plus the defensive list below."] = "「可驅散」指魔法或激怒，再加上下方的防禦清單。"
L["Categories"] = "類別"
L["These lists decide which auras count as defensive or as crowd control, for both sorting and border color."] = "這些清單決定哪些光環算是防禦或控制效果，同時影響排序與邊框顏色。"
L["Extra Crowd Control"] = "額外控制效果"
L["Added on top of the built-in crowd control list."] = "在內建的控制效果清單之上追加。"
L["Only the spells listed below are shown, never anything else."] = "僅顯示下方列出的法術，絕不顯示其他內容。"
L["Show auras on friendly nameplates. Only the spells listed below are ever shown: an ally in a raid carries dozens and any other rule fills the screen."] = "在友方名條上顯示光環。僅會顯示下方列出的法術：團隊中的友方身上有數十個光環，其他任何規則都會塞滿畫面。"
L["Tints plain debuffs by their dispel type (Magic, Curse, Disease, Poison). Crowd control and defensives keep their own color."] = "依驅散類型（魔法、詛咒、疾病、中毒）為一般減益上色。控制效果與防禦效果保留自己的顏色。"
L["Uses the combat log to guess casts on units you are not targeting. Less accurate than target, focus, mouseover, and arena enemies above."] = "利用戰鬥記錄推測你未選取單位的施法。準確度低於上方的目標、專注目標、滑鼠指向與競技場敵人。"
L["Sync From Native Cast Bar"] = "與原生施法條同步"
L["On supported clients, use the game's built-in nameplate cast bar for better timing. Safe to leave enabled."] = "在支援的客戶端上，使用遊戲內建的名條施法條以取得更準確的時間。可安全維持啟用。"
L["Native Progress Sync for Off-Target Casts"] = "非目標施法的原生進度同步"
L["When available, off-target combat-log castbars sync their progress from Blizzard's native castbar."] = "在可用時，以戰鬥記錄為基礎的非目標施法條會從暴雪原生施法條同步進度。"
L["showVKeyCastbar Mode"] = "showVKeyCastbar 模式"
L["auto: keep client value. force_on/off: DragonUI sets showVKeyCastbar while nameplates module is active."] = "auto：保留客戶端數值。force_on/off：名條模組啟用期間由 DragonUI 設定 showVKeyCastbar。"
L["Auto (Do Not Force)"] = "自動（不強制）"
L["Force On"] = "強制開啟"
L["Force Off"] = "強制關閉"
L["Quest"] = "任務"
L["Quest Icons"] = "任務圖示"
L["Show Quest Icons"] = "顯示任務圖示"
L["Show kill/loot icons over your quest-objective mobs. Without awesome_wotlk, only your target, mouseover and focus show them."] = "在任務目標怪物上方顯示擊殺/拾取圖示。若無 awesome_wotlk，僅有你的目標、滑鼠指向與專注目標會顯示。"
L["Pointer Mode"] = "指標模式"
L["Show a single quest marker on any objective mob instead of separate kill/loot icons."] = "在任何目標怪物上顯示單一任務標記，而非分開的擊殺/拾取圖示。"
L["Kill Icon"] = "擊殺圖示"
L["Choose the icon shown for kill objectives."] = "選擇擊殺目標所顯示的圖示。"
L["Sword"] = "劍"
L["Skull"] = "骷髏"
L["Elite"] = "精英"
L["Elite Kill Icon"] = "精英擊殺圖示"
L["Show a distinct icon on elite and rare kill objectives."] = "為精英與稀有擊殺目標顯示不同的圖示。"
L["Loot Icon"] = "拾取圖示"
L["Choose the icon shown for loot/collect objectives."] = "選擇拾取/收集目標所顯示的圖示。"
L["Bag"] = "背包"
L["Chest"] = "寶箱"
L["Pointer"] = "指標"
L["Pointer Icon"] = "指標圖示"
L["Off"] = "關閉"
L["Size"] = "大小"
L["Icon"] = "圖示"
L["Test Preview"] = "測試預覽"
L["Preview Icon"] = "預覽圖示"
L["Force one icon on all enemy nameplates so you can position and size it. Set to Off when done."] = "在所有敵方名條上強制顯示一個圖示，方便你調整位置與大小。完成後請設為關閉。"
L["DragonUI (Custom)"] = "DragonUI（自訂）"

-- Character Panel, Pets & Mounts and diagnostics
L["Modern reskin of the Blizzard character window."] = "為暴雪的角色面板換上現代外觀。"
L["Enable Character Panel"] = "啟用角色面板"
L["Apply the DragonUI reskin to the character window."] = "對角色視窗套用 DragonUI 外觀。"
L["Show your class icon in the portrait instead of your character's face."] = "在頭像處顯示職業圖示，而非角色臉孔。"
L["Class-Colored Level Text"] = "職業色等級文字"
L["Pets & Mounts"] = "寵物與坐騎"
L["A dedicated window for your mounts and companion pets, replacing the old Pet tab of the character window."] = "專門的坐騎與寵物視窗，取代角色視窗舊的寵物標籤。"
L["Enable Pets & Mounts"] = "啟用寵物與坐騎"
L["Add the Pets & Mounts micro menu button and its window."] = "新增寵物與坐騎的微選單按鈕及其視窗。"

-- Character Panel, Pets & Mounts and diagnostics
L["Commands: /dragonui, /dui, /pi \226\128\148 /dragonui edit (editor) \226\128\148 /dragonui help"] = "指令：/dragonui、/dui、/pi — /dragonui edit（編輯器）— /dragonui help"
L["Color the class name in the \"Level X Race Class\" line."] = "為\"等級 X 種族 職業\"一行中的職業名著色。"
L["Auto-show bars: Stance (Warriors/Druids/DKs) \226\128\162 Pet (Hunters/Warlocks/DKs) \226\128\162 Vehicle (All classes) \226\128\162 Totem (Shamans)"] = "自動顯示動作條：姿態（戰士/德魯伊/死騎）• 寵物（獵人/術士/死騎）• 載具（所有職業）• 圖騰（薩滿）"
L["Visual enhancements that add Dragonflight-style polish to the UI. These are optional \226\128\148 disable any you don't want."] = "為介面增添 Dragonflight 風格的視覺優化。均為選用 — 不想要的可以關閉。"
L["Detached \226\128\148 positioned freely via Editor Mode"] = "分離 — 在編輯模式中自由擺放"
L["Attached \226\128\148 follows Target frame"] = "附著 — 跟隨目標框體"
L["Attached \226\128\148 follows Focus frame"] = "附著 — 跟隨焦點框體"
L["Power Bar \226\128\148 Players Only"] = "能量條 — 僅玩家"

-- ============================================================================
-- PANELS TAB / GEAR SUMMARY
-- ============================================================================

L["Panels"] = "面板"
L["Character"] = "角色"
L["Stats Sidebar"] = "屬性側欄"
L["The headline numbers above the stat categories."] = "屬性分類上方的主要數值。"
L["Show Item Level"] = "顯示裝備等級"
L["Show the average item level of your equipped gear."] = "顯示你已裝備物品的平均裝備等級。"
L["Show GearScore"] = "顯示GearScore"
L["Show the GearScore of your equipped gear."] = "顯示你已裝備物品的GearScore。"
L["Set which stat and which combat panel lead the list:"] = "選擇哪項屬性和哪個戰鬥面板排在清單最前:"
L["Highlight Main Stat"] = "高亮主屬性"
L["The attribute your class is built around."] = "你的職業所依賴的屬性。"
L["Auto (by class)"] = "自動(依職業)"
L["Combat Statistics"] = "戰鬥數據"
L["Which one leads the list: Melee, Ranged, or Spell."] = "哪個排在清單最前:近戰、遠程或法術。"
L["Opens the window without the micro menu."] = "無需微型選單即可開啟視窗。"
L["Toggle Pets & Mounts"] = "開關寵物與坐騎"
L["Click, then press the key to bind. Press Escape to clear it."] = "點擊後按下要繫結的按鍵。按Esc清除。"
L["Key bindings cannot be changed in combat."] = "戰鬥中無法變更按鍵繫結。"
L["Hide Model Controls"] = "隱藏模型控制按鈕"
L["Hide the rotate, zoom and reset buttons over the character model."] = "隱藏角色模型上方的旋轉、縮放和重設按鈕。"
L["Keep the Reset Button"] = "保留重設按鈕"
L["Leave the reset button on its own while the rest of the model controls stay hidden."] = "在其餘模型控制按鈕保持隱藏時，只留下重設按鈕。"

-- ============================================================================
-- DAMAGE METER SKIN (DETAILS!)
-- ============================================================================

L["Damage Meter Skin"] = "傷害統計外觀"
L["|cffff5555Details! is not installed.|r This module skins the Details! Damage Meter, it is not a meter of its own - with Details! absent there is nothing to skin."] = "|cffff5555未安裝Details!。|r 本模組用於美化Details!傷害統計的外觀，本身並非統計工具 - 沒有Details!便無可美化。"
L["A retail-styled theme for |cffffcc55Details!|r, drawn with art from retail's own damage meter: a gold-titled header bar, class-coloured bars on a near-invisible panel, and abbreviated numbers. It is registered with Details! at login, so it also appears in Details!' own skin list under |cffffcc55DragonUI|r."] = "為|cffffcc55Details!|r打造的正式版風格主題，使用正式版傷害統計的美術資源：金色標題列、近乎隱形面板上的職業配色條，以及縮寫數字。主題會在登入時註冊至Details!，因此也會以|cffffcc55DragonUI|r之名出現在Details!自己的外觀清單中。"
L["Enable Damage Meter Skin"] = "啟用傷害統計外觀"
L["Registers the DragonUI theme with Details! and puts your chosen skin back after a reload. Turning it off hands every window back to Details!' own chrome."] = "在Details!中註冊DragonUI主題，並在重載介面後還原你選擇的外觀。關閉後所有視窗恢復Details!內建外觀。"
L["Apply the DragonUI Skin"] = "套用DragonUI外觀"
L["Switches every Details! window to the skin and sets K/M number abbreviation. Window size and position stay yours - use Details!' own scale slider for those. Your choice is remembered and put back after a reload; picking another skin in Details! ends that. Run this again after you customise something in Details! and want the theme back."] = "將所有Details!視窗切換為此外觀並啟用K/M數字縮寫。視窗大小與位置仍由你掌控 - 請使用Details!內建的縮放滑桿。你的選擇會被記住並於重載後還原；在Details!中選擇其他外觀即會結束。若你在Details!中進行過自訂並想取回主題，請再次執行。"
L["Run |cffffcc55/duidetails|r to apply it from chat."] = "輸入|cffffcc55/duidetails|r即可從聊天視窗套用。"

-- WORLD MAP TAB
-- ============================================================================

L["World Map"] = "世界地圖"
L["Retail-styled world map with breadcrumb navigation, drag-to-resize and quest log side panel. Requires a full /reload after toggling."] = "帶有導覽列、拖曳調整大小和任務記錄側邊面板的正式版風格世界地圖。切換後需完整 /reload。"
L["Enable World Map"] = "啟用世界地圖"
L["Activates the modern world map frame. Requires /reload to take effect."] = "啟用現代世界地圖框架。需要 /reload 生效。"
L["Enable Fog Reveal"] = "啟用迷霧揭示"
L["Draw grey tint over undiscovered zones using account-wide exploration data. Each alt sees your main's exploration immediately."] = "使用帳號範圍內的探索資料在未發現區域上繪製灰色遮罩。"
L["Enable Wheel Zoom"] = "啟用滾輪縮放"
L["Mouse wheel zooms and pans the map canvas."] = "滑鼠滾輪縮放和平移地圖畫布。"
-- World Map
L["World Map"] = "世界地圖"
L["Retail-style world map with breadcrumb navigation and a quest log side panel."] = "正式服風格的世界地圖，具備路徑導覽與任務記錄側邊欄。"
L["Enable World Map"] = "啟用世界地圖"
L["Apply the DragonUI reskin to the world map."] = "將 DragonUI 外觀套用至世界地圖。"
L["Show Undiscovered Areas"] = "顯示未探索區域"
L["Draw the map art of areas you have not explored yet, dimmed."] = "以暗色繪製尚未探索區域的地圖圖像。"
L["Show Landmarks"] = "顯示地標"
L["Show towns, flight points and other landmark pins on the map."] = "在地圖上顯示城鎮、飛行點與其他地標。"
L["Show Dungeon Entrances"] = "顯示地城入口"
L["Show dungeon and raid entrance pins on zone maps."] = "在區域地圖上顯示地城與團隊副本入口。"
L["Show Graveyards"] = "顯示墓地"
L["Show graveyard pins on zone maps."] = "在區域地圖上顯示墓地。"
L["Show Flight Points"] = "顯示飛行點"
L["Show flight master pins on zone maps."] = "在區域地圖上顯示飛行管理員。"

-- Iconic
L["Iconic"] = "Iconic"
L["Item icons in chat, merchant improvements, and enhanced item tooltips."] = "聊天物品圖示、商人介面改進及增強物品滑鼠提示。"

-- Item Loot (Loot Toast)
L["Loot Toast"] = "拾取提示"
L["Pretty loot toast alert notifications with custom textures."] = "具有自訂材質的美觀拾取彈窗提示。"

-- About Authors
L["Worked on by MiiKiis for the WoW Hollow server."] = "由 MiiKiis 為 WoW Hollow 伺服器開發。"
L["Original work by Neticsoul, with contributions by PentSec and the community."] = "基於 Neticsoul 的原作，以及 PentSec 和社群的貢獻。"
